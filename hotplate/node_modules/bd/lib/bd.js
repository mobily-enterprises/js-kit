...
hooks.run = function( done ){
  ...
  var Store = hotplate.getModule('hotMongoStores').Store;
  var declare = hotplate.getModule('hotMongoStores').declare;
  var e = hotplate.getModule('hotMongoStores').Errors;


  var OptionsStore = declare( Store,  {
    paramIds: [ 'optionId' ],
    checkId: function( id ) { return !! id.match(/^[0-9]+$/) },
    handleGet: true,
    checkPermissionsGet: function( cb ){
      if( this.req.params.optionId == '44' ){
        cb( new e.ForbiddenError );
      } else {
        cb( null );
      }
    },
  ...
  app.get( '/call/:workspaceIdCall/Options/:optionId', Store.makeGet( OptionsStore ) );
  console.log( Store.makeGet( OptionsStore ) );
