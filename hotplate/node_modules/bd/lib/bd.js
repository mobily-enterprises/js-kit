...
hooks.pageElementsPerPage = function( done, elements, req, pageName ){
  ...
  if( pageName  === 'hotDojoAppContainer/container' ){
    ...
    User.findOne({ _id: req.application.userId } , function( err, u){
      resUtil.checkFindOneResponse( err, u, done, function(){
        u = u.toObject();
        Workspace.findOne({ _id: req.application.workspaceId  }, function( err, w ){
          resUtil.checkFindOneResponse( err, w, done, function(){
            w = w.toObject();
            Workspace.findOne({ _id: req.application.workspaceId , 'access._id': req.application.userId } , 
                              ...
                              , function( err, access){
              resUtil.checkFindOneResponse( err, access, done, function(){
                access = access.toObject();


                // Settings the "artificial" ids
                u.settings._id = 'self';
                w.settings._id = 'self';
                access.access[0].settings._id = 'self';
                ...
                // Sending the page out
                  ...
                  moduleName: 'bd', result: {
                    ...
                    csses: ['bd.css'],
                    vars: [
                      { name: 'userSettings', value: u.settings },
                      { name: 'accessSettings', value:access.access[0].settings },
                      { name: 'workspaceSettings', value:w.settings } 
                    ], 
                  }
                });
              });
...
function putWorkspaceConfig( req, res, next ){
  ...
  if( resUtil.parametersAreEmpty(req.body, ['longName', 'tag'], errors, next) ) return; 
  ...
  Workspace.findByIdAndUpdate( req.application.workspaceId, updateObject, function( err, doc ){
    resUtil.checkUpdateResponse( err, doc, next, function(){
      sendResponse( res );
    });
  });
...
function putAccessConfig( req, res, next ){
  ...
  // if( resUtil.parametersAreEmpty( req.body, ['localEmail'], errors, next ) ) return; // FOR FUTURE NEEDS
  ...
  if( resUtil.respondIfErrors( errors, next ) ) return;
...
function getUserConfig( req, res, next ){
  ...
  User.findOne({ _id: req.application.userId } , function( err, doc){
    resUtil.checkFindOneResponse(err, doc, next, function(){
...
function putUserConfig( req, res, next ){
  ...
  // Actually update the object on the database
  User.update( { _id: req.application.userId }, updateObject).exec(function( err, doc){ 
    resUtil.checkUpdateResponse( err, doc, next, function(){
      sendResponse( res );
    });
  });
