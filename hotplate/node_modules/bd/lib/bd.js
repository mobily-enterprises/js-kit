...
function getWorkspaceConfig( req, res, next ){
  ...
  // Check the ID
  if( ! checkObjectIdAsync( req.params.workspaceId, "Workspace ID", next )) return;
...
function putWorkspaceConfig( req, res, next ){
  ...
  if( resUtils.parametersAreEmpty(req.body, ['longName', 'tag'], next) ) return; 

  // Stop here if an illegal parameter was added
  // TODO: make it possible to set parametersAllowed.defaults and use them
  if( resUtils.parametersAllowed(req.body, [ 'longName', 'tag', '_id', '__v', '_tabId' ], next) ) return; 
  ...
  workspaces.findOne( {_id: ObjectId( req.params.workspaceId ) }, function( err, doc ){
    resUtils.checkFindOneResponse( err, doc, next, function(){
      ...
      perms.checkPermissions( req, next, 'workspaceConfig/put', req.body, doc, function(){
       ...
       // Al clear: make up the update object
       var updateObject = resUtils.makeUpdateObject('settings.', req.body );
 
        workspaces.findAndModify( { _id: ObjectId( req.params.workspaceId ) }, {}, updateObject, {new: true}, function( err, doc ) {
