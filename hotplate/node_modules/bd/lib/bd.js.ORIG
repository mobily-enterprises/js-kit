...
function putWorkspaceConfig( req, res, next ){
  ...
  Workspace.findOneAndUpdate( req.params.workspaceId, updateObject, function( err, doc ){
    resUtils.checkFindOneResponse( err, doc, next, function(){
      sendResponse( res );
...
function putAccessConfig( req, res, next ){
  ...
  Workspace.findOne({ _id: req.application.workspaceId , 'access._id': req.params.userId } ,
                    { 'access': { $elemMatch: { _id: req.params.userId } } }, { strict: true }
                   , function( err, doc){
    resUtils.checkFindOneResponse(err, doc, next, function(){
      doc = doc.toObject();
      doc.access[0].settings._id = req.params.userId; // Artificial ID, will match the user's
      var r = doc.access[0].settings;

      // Actually update the object on the database
      Workspace.findOneAndUpdate( { _id: req.application.workspaceId, 'access._id': req.params.userId },          
                               updateObject)
        ...
        resUtils.checkFindOneResponse( err, doc, next, function(){
          ...
          // We are forced to re-run the search, as it's actually impossible to run the update()
          // query above as a findOneAndUpdate() _and_ somehow know the right index for access._id
          // but hey...
