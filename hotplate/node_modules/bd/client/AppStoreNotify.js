...
define([
  ...
  topic.subscribe('storeUpdate', function( from, message){
    ...
    if( message.remote ){
      // Inject the data into the store's cache
      stores(message.storeName).memCache.put( message.object, { dontPublish: true } );
  ...
  topic.subscribe('storeRemove', function( from, message){
    ...
    if( message.remote ){
      // Inject the data into the store's cache
      stores(message.storeName).memCache.remove( message.objectId, { dontPublish: true } );
