...
function registerSync( req, res, next ){
  ...
  registerTab( req.application.userId, req.application.workspaceId, function( err, doc ){
    if( err ){
     ...
     messages.forEach( function(item) {
        if( item.type !== 'reset' ){
          if( item.toType === 'toMyTabs'){
            sendToTabsOfUser( req.application.userId, tabId,      req.application.userId, item.type, item.message );
          }
          if( item.toType === 'toEverybody'){
            sendToTabsOfWorkspace( req.application.workspaceId, tabId,      req.application.userId, item.type, item.message );
          }
        }
      });
        ...
        // Nothing returned: tab doesn't exist, creating a new one and letting client know
          ...
          registerTab(  req.application.userId, req.application.workspaceId, function( err, doc ){
            if( err ){
              ...
              var messages = [];
              messages.push( { from: req.application.userId, type: 'reset', message: { tabId: doc._id } } );
