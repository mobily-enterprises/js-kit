...
the session variables `req.session.loggedIn`, `req.session.lastLogin`, `req.session.login` and `req.session.userId` will 
NOT be set. Having everything here makes things like logging much easier
*/
  ...
  hotplate.invokeAll('enrichAuthAccessSchema', Access, function(err, results){
    ...
    hotplate.invokeAll('enrichAuthWorkspaceSchema', Workspace, function(err, results){
      ...
      var User = new Schema({
        ...
        email         : { type: String }, // Always lowercase
      });
...
paramWorkspaceIdPages = function( req, res, next, workspaceId ){
  ...
  Workspace.findOne({ _id: mongoose.Types.ObjectId(workspaceId), 'access._id':req.session.userId }, 
                    { name: 1, access: { $elemMatch: { _id: req.session.userId } } } ,
                    function(err, doc){
    ...
    if(err){
      ...
      if(doc){
        ...
        req.application.token = doc.access[0].token;
...
var paramTokenCall = function( req, res, next, token ){
  ...
  Workspace.findOne({ 'access.token': token } , 
                    { name: 1, access: { $elemMatch: { token: token }  } }, // FIXME: UNTESTED
                    function(err, doc){
    if(err){
      next( new e.RuntimeError( err ) );
    } else {
      if(! doc ){
        ...
        accessEntry = doc.access.filter[0];
...
var postWorkspacesAnon = function(req, res, next){
  ...
  if( ! Validators.email(req.body.email) ){
    errors.push( {field: 'email' , message: Validators.email(false) });
  }
              ...
              if(err){
                ...
                makeToken( function(err, token) {
                  if(err){
                    ...
                    w.access = {  _id: u._id, token:token };
                    w.save( function(err){
                      if(err ){
                        next( new e.RuntimeError( err ) );
                      } else{
...
var postRecoverAnon = function(req, res, next){
  ...
  // Enrich the `errors` object if parameters are empty strings
  parametersNotEmpty(req.body, [ 'email' ], errors);
