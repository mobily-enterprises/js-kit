...
hooks.init = function( done ){
  ...
  // Soon to be deleted, after Mongoose eradication
...
var postWorkspacesAnon = function(req, res, next){
  ...
  users.findOne( { login: req.body.login.toLowerCase() }, function(err, doc){
    ...
    if(err ){
      ...
      workspaces.findOne({ name: req.body.workspace.toLowerCase() }, function(err, doc){
        if(err ){
          next(new e.RuntimeError( err ) );
        } else {
          if(doc){
...
var postRecoverAnon = function(req, res, next){
  ...
  users.findOne( { login: req.body.registerEmail.toLowerCase }, function(err, doc){
    // Log database error if it's there
    if(err ){
      next(new e.RuntimeError( err ) );
    } else {
...
var postLoginAnon = function(req, res, next){
  ...
  users.findOne( { login: req.body.login.toLowerCase() }, function(err, docUser){
    // Log database error if it's there
    if(err ){
      next(new e.RuntimeError( err ) );
    } else {
              ...
              if( typeof(req.params.workspaceName) !== 'undefined'){
                ...
                users.findOne( { login: req.body.login.toLowerCase() } , function( err, docUser){
                  if(err){
                    ...
                    workspaces.findOne( { 'name': req.params.workspaceName.toLowerCase(), 'access._id' : docUser._id }, function( err, docWorkspace ){
                      if(err ){
                        next(new e.RuntimeError( err ) );
                      } else {
                        if(docWorkspace){
