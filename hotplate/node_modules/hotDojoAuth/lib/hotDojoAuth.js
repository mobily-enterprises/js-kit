...
here makes things like logging easier
*/
...
var pageLoginWorkspace = function( req, res, next ){
  ...
  if( typeof( req.params.workspaceName ) != 'undefined' ){
    ...
    Workspace.findOne({ name: req.params.workspaceName}, function(err, doc){
      if(err){
         res.status = 500;
...
var postWorkspacesAnon = function(req, res, next){
  ...
  console.log("MERC -----------------------");
  console.log(hotplate.getModule('hotSharedCode').sharedFunctions );
  ...
  if(errors.length){
    ...
    console.log(e.ValidationError); 
    console.log("%j", hotplate.getModule('hotError').errors);
...
/* 
// OLD, INCOMPLETE FUNCTION THAT WORKED WITH THE WORKSPACE NAME. UNUSED.
// IT'S TO BE REDONE CHECKING THAT THE USER IS LOGGED IN ANYWAY
