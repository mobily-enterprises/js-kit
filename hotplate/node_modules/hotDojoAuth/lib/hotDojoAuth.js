...
hooks.init = function( done ){
  ...
  hotplate.invokeAll('enrichAuthTables', function(err, results){
    done( null );
  });

}
hooks.init.invokes = [ 'enrichAuthTables' ];
              ...
              if( typeof(req.params.workspaceName) !== 'undefined'){
                ...
                User.findOne( { login: req.body.login.toLowerCase() } , function( err, docUser){
                  if(err){
                    ...
                    Workspace.findOne( { 'name': req.params.workspaceName.toLowerCase(), 'access.userId' : docUser._id }, function(err, docWorkspace){
                      if(err ){
                        next(new e.RuntimeError( err ) );
                      } else {
                        if(docWorkspace){
