var dummy
  ...
  , Validators = hotplate.getModule('hotSharedValidators').hotHooks.sharedFunctions()
;
...
hooks.run = function(){
  ...
  // Login screen. Will load login.js and rrl.js
  ...
  app.get('/pages/register' , function(req, res, next){
    var extraJses = (new hotPage.Jses).add('hotDojoAuth', 'register.js');
    var extraCsses = (new hotPage.Csses).add('hotDojoAuth', 'rrl.css');
    result = hotPage.processPageTemplate( { jses:extraJses, csses:extraCsses, body: '<body class="claro"><div id="registerForm"></div></body>' } );
    res.send( result );
    logger.log( { message: "Register page served" } );
  });


  app.get('/pages/pick' , function(req, res, next){

    // console.log("WS: %j" , req.session );
    ...
    Workspace.find( { 'access.userId': req.session.userId }, function(err, docs){
      if( err ){
        ...
        if( list.length == 0){
          var extraCsses = (new hotPage.Csses).add('hotDojoAuth', 'rrl.css');
          result = hotPage.processPageTemplate( { csses:extraCsses, body: '<body class="claro"><div id="pickForm">No workspace to pick!</div></body>' } );
          res.send( result );
        ...
        // There is a proper list of workspaces -- let the user pick
          ...
          var extraCsses = (new hotPage.Csses).add('hotDojoAuth', 'rrl.css');
          result = hotPage.processPageTemplate( { csses:extraCsses, body: '<body class="claro"><div id="pickForm">' + pickList + '</div></body>' } );
          res.send( result );
  ...
  app.param( 'workspaceNamePages', paramWorkspaceNamePages);   // Used by /pages/login/WORKSPACENAME
  app.param( 'workspaceIdPages',   paramWorkspaceIdPages);     // Used by /pages/ws/WORKSPACEID
  ...
  app.post( '/call/logout',         getLogoutUser );
...
hooks.stores = function(){
  return {
    ...
    logout        : { target: '/call/logout',         idProperty: '_id', sortParam: 'sortBy', },
  }
}
...
var paramWorkspaceNamePages = function( req, res, next, workspaceName ){
...
paramWorkspaceIdCall = function( req, res, next, workspaceId ){
  ...
  // Check that the workspaceId is in a valid format
  if( ! htplate.objectIdCheck(workspaceId) ) {
