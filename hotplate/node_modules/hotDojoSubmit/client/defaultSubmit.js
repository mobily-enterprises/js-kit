    ...
    if( type == 'ok') {
      ...
      return  function(res){
         ...
         if(! res){
          ...
          // There is a message: display it (likey to be a confirmation)
          if( res.message && alertBar ){
            alertBar.set('message', res.message );
            alertBar.show(2000); // OK message for 2 seconds
          }
        ...
        switch(err.status){
          ...
          case 403:
            ...
            if(! noLogin){
              ...
              topic.publish( 'hotDojoSubmit/authNeeded' );
            }
          ...
          default:
            ...
            var status = typeof(err.status) === 'undefined' ? 'none' : err.status;

            // Show the error at application level
            topic.publish( 'globalAlert', 'Application error: ' + message + ' Status: ' + status, 5000 );

            // Cancel the submit button
            button ? button.cancel() : null;

            logger("Respose came back with error: " + res.message + ' Status: ' + err.status );
