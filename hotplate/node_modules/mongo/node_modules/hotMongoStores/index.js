...
var Store = declare( null,  {
  ...
  // Stock "fetchDoc" function. It always works, and returns { _id: 'value' }
  fetchDoc: function( cb ){
    var doc = {};
    ...
    // Defining this.fetchedDoc and (optional) this.fetchedOriginalDoc as API
    this.fetchedDoc = doc;
  ...
  prepareDocBeforeSending: function( doc, cb ){
    cb( null, doc );
  },


  createDoc: function( obj, cb ){
    cb( null, obj );
  },

  insertDoc: function( obj, cb ){
    cb( null, cb );
  },

  deleteDoc: function( cb ){
    cb( null );
  },
  ...
  // Maybe keep, but call callback with parameters (1, ..., -1 )
  _sendErrorCheck: function( err, cb ){
    if( err ){
      this._sendError( err );
    } else {
     cb();
  ...
  _sendError: function( error ){
    ...
    this.res.send( responseStatus, responseBody );
    this.logError( error );
  },
  ...
  _makeGet: function( req, res, next ){
    ...
    if( ! self.handleGet ){
      self._sendError( new e.NotImplementedError( ) );
      return;
    }
    ...
    if( errors.length ){
      self._sendError( new e.BadRequestError( null, errors ) );
      return;
    }

    // Fetch the doc.
    self.fetchDoc( function( err ){
      if( err ){
        self._sendError( err );
      } else {
        if( ! self.fetchedDoc ){
          self._sendError( new e.NotFoundError());
        } else {

          // If fetchDoc() only set this.fetchedDoc, manually set
          // this.fetchedOriginalDoc
          if( typeof( this.fetchedOriginalDoc ) === 'undefined' ){
            self.fetchedOriginalDoc = self.fetchedDoc;
          }

          // Check the permissions 
          self.checkPermissionsGet( function( err ){
            if( err ){
              self._sendError( err );
            } else {

              // "preparing" the doc. The same function is used by GET for collections 
              self.prepareDocBeforeSending( self.fetchedDoc, function(err, doc ){
                if( err ){
                  self._sendError( err );
                } else {
                   
                  // Send "prepared" doc
                  self.res.json( 200, doc );
                } 
              })
            }
          })
  ...
  checkPermissionsPost: function( cb ){
    cb( null, true );
  },
  checkPermissionsPut: function( cb ){
    cb( null, true );
  },
  checkPermissionsGet: function( cb ){
    cb( null, true );
  },
  checkPermissionsDelete: function( cb ){
    cb( null, true );
  },
...
Store.makeAll = function( app, url, idName, Class ){
  app.get( Store.makeGet( url + idName, Class ));
}
