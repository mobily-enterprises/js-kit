
require([
  'dojo/topic',
  'dojo/_base/window',

  'hotplate/hotDojoWidgets/AlertBar',
  //"hotplate/hotDojoHooks/Sync",
  //"hotplate/hotDojoHooks/SyncSubscriber!hotplate/hotHooks/startup/after/registerForm|mainAlertBar",

  "hotplate/hotDojoHooks/SyncTopic",
  "hotplate/hotDojoHooks/SyncSubscriber!hotplate/hotHooks/postCreate/after/registerForm|mainAlertBar",

  ], 
  function(
    topic
  , win

  , AlertBar
  , SyncTopic
){
  // console.log("In mainAlertBar.js");

  globalAlertBar = new AlertBar( { background: '#FF4444' } );
  globalAlertBar.placeAt( document.body, 'first' );
  globalAlertBar.startup();

  topic.subscribe('globalAlert', function( message, displayFor ){
    displayFor = displayFor || 2500;
    globalAlertBar.set( 'message', message );
    globalAlertBar.show( displayFor );
  });

  //// This is here to test out SyncTopic
  ////Sync.incSubscriber('hotplate/hotHooks/postCreate/after/registerForm');
  //topic.subscribe('hotplate/hotHooks/postCreate/after/registerForm', function( widget ) {
  //  console.log("Subscribe in manAlertBar happened ");
  //});
  //SyncTopic.ready('hotplate/hotHooks/postCreate/after/registerForm');

});


