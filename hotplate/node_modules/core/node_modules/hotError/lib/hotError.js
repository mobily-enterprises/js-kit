...
exports.hotErrorHandler = function( err, req, res, next){
  ...
  if( err.sendResponse ) {
      ...
      sendResponse( res, { message: message, errors: err.errors }, httpError );
  } else {
    next(err);
  }
}
...
hooks.definedErrors = function( done ){
  ...
  vars['BadRequestError'] = function( message, errors ){
    this.httpError = 400;
  ...
  util.inherits(vars['BadRequestError'], Error);
  ...
  vars['UnauthorizedError'] = function( message ){
    this.httpError = 401;
  ...
  util.inherits(vars['UnauthorizedError'], Error);
