...
hooks.init = function(){
  ...
  hotplate.invokeAll('definedErrors').forEach(function(entry){
    var moduleName = entry.moduleName;
    var errorsFromModule = entry.result;
...
exports.hotErrorHandler = function( err, req, res, next){
  ...
  // Log the error using hotLogger's logger (if it's defined)
  if( logger = hotplate.getModule('hotServerLogger') ) {
  ...
  if( err.sendResponse ) {
      ...
      protocol.sendResponse( res, { ack: 'ERROR', message: err.message, errors: err.errors, status: httpError } );
  } else {
    next();
  }
}
