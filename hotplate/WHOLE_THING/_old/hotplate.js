 ...
 * Add ability to register a module later (change the load function so that it calls register, and then
  ...
  -- Change loading procedure so that it does a straight require, no main.js involved
  -- Change exports.init into exports.hooks.init() so that nothing of the main namespace is taken
  -- Definitely move the logger out of the equation, turn it into a normal module. NOTE: now that init() _always_ happens
     after _every_ module is loaded/registered, this is easy
...
function Hotplate() {
  ...
  this.modulesAreInitialised = false; // True if modules are loaded
...
Hotplate.prototype.initModules = function(){
 ...
 // Initialise all loaded modules, calling their init() functions
  ...
  // added by the loaded modules. The "true" at the end means that
  // processPageTemplate() will leave the placeholders where they are
 ...
 * Load a module
 *
 * Load a module into Hotplate's module registry
 * 
...
Hotplate.prototype.invokeAll = function(){
  ...
  for(var moduleName in this.modules){
    ...
    if( typeof( module.hooks ) === 'object' && typeof( module.hooks[hook] ) === 'function' ){
      ...
      results.push( module.hooks[hook].apply( module, Array.prototype.splice.call(arguments, 1) ) );
    }
