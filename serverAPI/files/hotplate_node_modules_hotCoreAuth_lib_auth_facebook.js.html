...
exports = module.exports = function( app, strategyName, stores ) {
  ...
  function(req, accessToken, refreshToken, profile, done) {
    ...
    stores.userStrategies.dbLayer.selectByHash( { conditions: { userId: req.session.userId, strategyId: &#x27;facebook&#x27; } }, function( err, res ){
      ...
      stores.userStrategies.dbLayer.selectByHash( { conditions: { field1: profile.id } }, { children: true }, function( err, res ){
        if( err ) return done( err, null );
        ...
        stores.userStrategies.apiPost( { userId: req.session.userId, strategyId: &#x27;facebook&#x27;, field1: profile.id }, function( err, res ){
          if( err ) return  done( err, null );

          stores.users.dbLayer.selectById( req.session.userId, function( err, user ){
            if( err ) return done( err, null );
  ...
  function( req, accessToken, refreshToken, profile, done ) {
    ...
    stores.userStrategies.dbLayer.selectByHash( { conditions: { strategyId: &#x27;facebook&#x27;, field1: profile.id } }, { children: true }, function( err, res ){
      if( err ) return done( err, null );
  ...
  function(req, accessToken, refreshToken, profile, done) {
    ...
    stores.userStrategies.dbLayer.selectByHash( { conditions: { strategyId: &#x27;facebook&#x27;, field1: profile.id } }, { children: true }, function( err, res ){
      ...
      stores.users.dbLayer.insert( {}, function( err, user ){
        ...
        stores.userStrategies.dbLayer.insert( { userId: user.id, strategyId: &#x27;facebook&#x27;, field1: profile.id }, function( err, res ){
          if( err ) return done( err, null );
