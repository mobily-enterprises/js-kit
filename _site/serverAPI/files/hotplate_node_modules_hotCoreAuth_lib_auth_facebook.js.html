...
exports = module.exports = function( app, strategyConfig, authStrategies, users, userStrategies, userLogins ) {
  ...
  function(req, accessToken, refreshToken, profile, done) {
    ...
    userStrategies.dbLayer.selectByHash( { conditions: { userId: req.session.userId, strategyId: &#x27;facebook&#x27; } }, function( err, res ){
      ...
      userStrategies.dbLayer.selectByHash( { conditions: { field1: profile.id } }, { children: true }, function( err, res ){
        if( err ) return done( err, null );
  ...
  function( req, accessToken, refreshToken, profile, done ) {
    ...
    userStrategies.dbLayer.selectByHash( { conditions: { strategyId: &#x27;facebook&#x27;, field1: profile.id } }, { children: true }, function( err, res ){
      if( err ) return done( err, null );
    ...
    userStrategies.dbLayer.selectByHash( { conditions: { strategyId: &#x27;facebook&#x27;, field1: profile.id } }, { children: true }, function( err, res ){
      ...
      if( res.length ){
        users.dbLayer.selectById( res[0].userId, function( err, user ){
          ...
          users.dbLayer.updateById( user.id, user, function( err, howMany){
            if( err ) return done( err, null );
  ...
  function(req, accessToken, refreshToken, profile, done) {
    ...
    userStrategies.dbLayer.selectByHash( { conditions: { strategyId: &#x27;facebook&#x27;, field1: profile.id } }, { children: true }, function( err, res ){
      ...
      users.dbLayer.insert( {}, function( err, user ){
        if( err ) return done( err, null );

        userStrategies.dbLayer.insert( { userId: user.id, strategyId: &#x27;facebook&#x27;, field1: profile.id }, function( err, res ){
          if( err ) return done( err, null );
