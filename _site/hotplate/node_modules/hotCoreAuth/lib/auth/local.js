...
var exports = module.exports = function( app, strategyConfig, makeResponder, authStrategies, users, userStrategies, userLogins ) {
  ...
  function(req, login, password, done) {
    ...
    userStrategies.apiGetQuery( { filters: { userId: req.session.userId, strategyId: 'local' } }, function( err, res ){
      if( err ) {
        done( err, null );
      } else {
        if( res.length ){
        ...
        userStrategies.apiGetQuery( { filters: { field1: login.toLowerCase() } }, function( err, res ){
  ...
  function(req, login, password, done) {
    ...
    userStrategies.apiGetQuery( { filters: { field1: login.toLowerCase(), field3: password } }, function( err, res ){
      if( err ) {
        done( err, null );
      } else {
        if( res.length ){
  ...
  function(req, login, password, done) {
    ...
    if( req.session.loggedIn ){
      ...
      userStrategies.apiGetQuery( { filters: { strategyId: 'local', field1: login.toLowerCase() } }, function( err, res ){
        if( err ) {
          done( err, null );
        } else {
          if( ! res.length ){
