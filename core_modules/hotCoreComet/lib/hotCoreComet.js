...
var hotCoreStore = require('hotplate/core_modules/hotCoreStore')
var debug = require('debug')('hotplate:hotCoreComet')
var WebSocketServer = require('ws').Server
var crypto = require('crypto')
var hotCoreServerLogger = require('hotplate/core_modules/hotCoreServerLogger')

var logger = hotCoreServerLogger
...
var conditionalTabDispatch = exports.conditionalTabDispatch = function (ce, selector, makeMessage, options, cb) {
  ...
  async.eachSeries(
      ...
      selector(ce, tab, tabSession, function (err, result) {
        ...
        if (result) {
          ...
          makeMessage(ce, tab, tabSession, ce.message, function (err, newMessage) {
            if (err) {
              logger.log({ error: err, system: false, logLevel: 3, message: 'Error running makeMessage', data: { ce: ce, tab: tab, tabSession: tabSession } })
              return cb(null)
            }
    ...
    function (err) {
      if (err) {
        logger.log({ error: err, system: true, logLevel: 3, message: "Error in conditionalTabDispatch's each cycle", data: { ce: ce, tabs: ce.tabs } })
      }
    ...
    async.eachSeries(
      ...
      function (sub, cb) {
        ...
        selector(ce, sub, tabSession, function (err, result) {
          if (err) {
            logger.log({ error: err, system: true, logLevel: 3, message: 'Error running selector', data: { ce: ce, sub: sub, tabSession: tabSession } })
            return cb(null)
          }
      ...
      function (err) {
        if (err) {
          logger.log({ error: err, system: true, logLevel: 3, message: "Error in conditionalSubDispatch's each cycle", data: { ce: ce, subs: subs } })
        }
...
hotplate.hotEvents.onCollect('serverCreated', 'hotCoreComet', hotplate.cacheable(function (server, done) {
  ...
  wss.on('connection', function connection (ws) {
    consolelog('CONNECTION ESTABLISHED........', ws)
      ...
      switch (message.type) {
        ...
        case 'subscribe':
          ...
          stores.tabSubscriptions.dbLayer.selectByHash({ tabId: tabId }, function (err, total) {
            ...
            if (total > 500) {
              consolelog('Too many subscriptions', total, message.tabId)
              return
            }
            ...
            stores.tabSubscriptions.dbLayer.insert(sub, function (err, registerRecord) {
              ...
              consolelog('OK subscription done!!!', total)
...
hotplate.hotEvents.onCollect('comet-event', function (ce, cb) {
  ...
  function selector (ce, tab, tabSession, cb) {
    tabSession = ce.connections[ tab.id ]
    if (!tabSession) return cb(null, false)
