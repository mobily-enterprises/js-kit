...
hooks.sharedFunctions = function( done ) {
  ...
  result.notEmptyString = function(str){
  ...
  result.onlyLetters = function(str){
  ...
  // Login validator
  result.login = function(str){

    // FIXME: Check that the input is indeed a string

    if(str === false) return "Can only contain letters, number and '.'";
    // console.log(str);
  ...
  // Workspace validator
  result.workspace = function(str){
    if(str === false) return "Can only contain letters and numbers";
    return ! str.match(/[^a-zA-Z0-9]/);
  }

  // Email validator
  // http://rosskendall.com/blog/web/javascript-function-to-check-an-email-address-conforms-to-rfc822
  result.email = function(str){
  ...
  done( null, { moduleName: 'hotCoreCommonValidators', result: result } );
