  ...
  getConfigStores( function( err, configStores ){
    ...
    async.each(
      ...
      function( storeName, cb ){
        ...
        hotplate.log("IN ITERATING FUNCTION FOR " + store.storeName ); 
        ...
        // It's a one-field set, it will only return ONE value. One record sets are the ones
        // 
        if( store.idProperty == 'workspaceId' || store.idProperty == 'userId' || store.idProperty == 'globalId'){
          ...
          store.apiGetQuery( { filters: filter }, function( err, docs ){
            ...
            for( var k in store.schema.structure ){
              ...
              if( typeof( v.default ) !== 'undefined' ) filter[ k ] = v.default;
            }

            // globalId is a special case: will use a post, since we don't
            // really care what the ID will be as there will only ever be 1 entry
            // and apiGetQuery won't set any filters when fetching config.
            if( store.idProperty === 'globalId' ){

              store.apiPost( filter, { killComet: true }, function( err, doc ){
                if( err ) return cb( err );
            ...
            // For workspaceId and userId, the ID is important: it will have a put
            // there. Note that workspaceId and userId have been set earlier   
            } else {
