    ...
    Object.keys( configStores.stores ).forEach( function( storeName ){
      ...
      function loadConfigStoreFunction( Store, storeObject ){
        ...
        return function( next ){
          hotplate.log("PRE-LOADING RECORD FOR " + storeObject.storeName ); 
          ...
          if( storeObject.idProperty == 'workspaceId' || storeObject.idProperty == 'userId' ){
            ...
            Store.GetQuery( { filters: filter }, function( err, docs ){
              if( err ) {
                ...
                if( docs.length !== 0 ){
                  hotplate.log("Record existed, all good for " + storeObject.storeName ); 
                  next( null, docs[ 0 ] );
                } else {
                  hotplate.log("Record didn't exist! Creating one for " + storeObject.storeName ); 
                  hotplate.log(filter ); 

                  // Adding defaults taken from the schema
                  for( var k in storeObject.schema.structure ){
                    var v = storeObject.schema.structure[ k ];
                    if( v.default ) filter[ k ] = v.default;
                  }

                  // Actually do the put
                  Store.Put( null, filter, { killComet: true }, function( err, doc ){
                    next( err, doc );
                  })
                }
              }
      ...
      if( storeObject.configStore ){
        ...
        if( storeObject.configStore.workspaceId || storeObject.configStore.userId ){
