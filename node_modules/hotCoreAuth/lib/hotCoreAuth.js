...
hotplate.hotEvents.on( 'stores', 'hotCoreAuth', hotplate.cachable( function( done ){
  ...
  hotCoreJsonRestStores.getMixins( function( err, hotMixins ){
    if( err ){
      done( err );
    } else {

      var HotStore = declare( [ JsonRestStore, hotplate.config.get( 'hotplate.DriverMixin'), hotMixins.StoreMixin ] );
      var HotSchema = declare( [ SimpleSchema, hotplate.config.get( 'hotplate.SchemaMixin'), hotMixins.SchemaMixin ] );
      ...
      stores.UserStrategies = declare( HotStore, {
      ...
      stores.UserLogins = declare( JsonRestStore, {
        ...
        driverGetDbQuery: function( params, body, options, cb ){
          ...
          stores.UserStrategies.GetQuery( { filters: { strategyId: 'local', field1: options.filters.login } }, function( err, res ){
            if( err ){
              cb( err, null );
            } else {
              if( res.length ){
