...
hotplate.config.set('hotCoreAuth', {
  ...
  redirectURLs: {
    success: {
      signin: path.join( hotplate.config.get( 'hotplate.dynamicUrlPrefix' ), '/auth/pick' ),
      recover: path.join( hotplate.config.get( 'hotplate.dynamicUrlPrefix' ), '/auth/pick'),
      register: path.join( hotplate.config.get( 'hotplate.dynamicUrlPrefix' ),'/auth/pick'),
      manager: '/',
    },

    fail: {
      signin: path.join( hotplate.config.get( 'hotplate.dynamicUrlPrefix' ), '/auth/welcome' ),
      recover: path.join( hotplate.config.get( 'hotplate.dynamicUrlPrefix' ), '/auth/welcome' ),
      register: path.join( hotplate.config.get( 'hotplate.dynamicUrlPrefix' ), '/auth/welcome' ),
      manager: '/',
    }
  },
  responsePage: basicResponsePage,
  appURL: path.join( hotplate.config.get( 'hotplate.dynamicUrlPrefix' ), '/app' ),
...
hotplate.hotEvents.on( 'stores', 'hotCoreAuth', hotplate.cachable( function( done ){
  ...
  hotCoreJsonRestStores.get( function( err, s ){
    if( err ){
      ...
      stores.Users = declare( HotStore, {
    
        // COMMON
        schema: new HotSchema({
          id:                  { type: 'id' },
    
          recoverToken:        { type: 'string', searchable: true, notempty: true },
      ...
      stores.UsersStrategies = declare( HotStore, {
        ...
        publicURL: '/users/:userId/strategies/',
        ...
        checkPermissionsDelete: function( params, body, options, doc, fullDoc, cb ){
          ...
          stores.UsersStrategies.GetQuery( { filters: { userId: doc.userId } }, function( err, queryDocs ){
      ...
      stores.AuthStrategies = declare( JsonRestStores, {
    
        schema: new SimpleSchema({
          id:        { type: 'string', isRequired: true, trim: 30 }  ,
        }),
        ...
        execGetDbQuery: function( params, body, options, cb ){
          var strategies = hotplate.config.get('hotCoreAuth.strategies');
          var doc;
          var docs = [];
      ...
      stores.Logins = declare( JsonRestStores, {
        ...
        execGetDbQuery: function( params, body, options, cb ){
          ...
          stores.UsersStrategies.GetQuery( { filters: { strategyId: 'local', field1: options.filters.login } }, function( err, res ){
            if( err ){
              cb( err, null );
            } else {
              if( res.length ){
                 //self.schema.makeId( res, function( err, id ){
                 // cb( null, [ { login: options.filters.login, id: id } ] );
                cb( null, [ { login: options.filters.login } ] );
                // });
              } else {
                cb( null, [ ] );
              }
      ...
      done( null, [ stores.Users, stores.UsersStrategies, stores.AuthStrategies, stores.Logins ] );
    ...
    if( err ){
            ...
            if( result.length == 0 ){
              ...
              if( ! user.recoverTokenCreated ){
                ...
                if( ! tokenIsGood ){
                  ...
                  stores.users.Store.Put( user.id, user, function( err, userNew ){
                    if( err ){
                      next( err );
                    } else {
      ...
      if( typeof( hotplate.config.get('hotCoreAuth.strategies' )) === 'object' ){
        Object.keys( hotplate.config.get('hotCoreAuth.strategies' )).forEach( function( strategyName ) {
          ...
          strategyMaker( app, strategyConfig, makeResponder, stores.AuthStrategies.Store, stores.Users.Store, stores.UsersStrategies.Store, stores.Logins.Store );
        });
...
hotplate.hotEvents.on( 'pageElementsPerPage', 'hotCoreAuth', function( req, pageName, done ){
