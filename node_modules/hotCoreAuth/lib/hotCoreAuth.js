    ...
    if( err ){
        ...
        checkPermissions: function( request, method, p, cb ){
          ...
          switch( method ){
            ...
            case 'delete':
              ...
              stores.usersStrategies.apiGetQuery( { conditions: { userId: doc.userId } }, function( err,queryDocs) {
      ...
      var Logins = declare( JsonRestStores, {
        ...
        implementQuery: function( request, cb ){
          ...
          stores.usersStrategies.apiGetQuery( { conditions: { strategyId: 'local', field1: request.options.conditions.login } }, function( err, res ){
            if( err ){
              cb( err, null );
            } else {
              if( res.length ){
  ...
  hotCoreStoreRegistry.getAllStores( function( err, stores ){
    ...
    if( err ){
      ...
      app.get('/recover/:recoverToken', function( req, res, next ){
        ...
        stores.users.apiGetQuery( { conditions: { recoverToken: req.params[ 'recoverToken' ] } }, function( err, result ){
          if( err ){
            done( err, null );
          } else {
            if( result.length == 0 ){
