...
hotplate.hotEvents.on( 'setRoutes', function( done ){
  ...
  hotCoreStoreRegistry.getAllStores( function( err, storesData ){
    if( err ){
      ...
      hotplate.app.get('/recover/:recoverToken', function( req, res, next ){
      ...
      if( typeof( hotplate.get('hotCoreAuth/strategies' )) === 'object' ){
        Object.keys( hotplate.get('hotCoreAuth/strategies' )).forEach( function( strategyName ) {
          ...
          strategyMaker( strategyConfig, makeResponder, stores.authStrategies.Store, stores.users.Store, stores.userStrategies.Store, stores.userLogins.Store );
        });
