...
var exports = module.exports = function( app, strategyConfig, makeResponder, authStrategies, users, userStrategies, userLogins ) {
  ...
  function(req, login, password, done) {
    ...
    userStrategies.apiGetQuery( { conditions: { userId: req.session.userId, strategyId: 'local' } }, function( err, res ){
      if( err ) {
        done( err, null );
      } else {
        if( res.length ){
        ...
        userStrategies.apiGetQuery( { conditions: { field1: login.toLowerCase() } }, function( err, res ){
          ...
          if( err ) {
            ...
            if( res.length > 0 && !editingSelf ){
              ...
              if( itsEdit ){
                ...
                userStrategies.apiPut( foundStrategyId, { userId: req.session.userId, strategyId: 'local', field1: login.toLowerCase(), field3: password }, function( err, res ){
                  if( err ) {
                    done( err, null );
                  } else {
                    complete( res.id );
  ...
  function(req, login, password, done) {
    ...
    userStrategies.apiGetQuery( { conditions: { field1: login.toLowerCase(), field3: password } }, function( err, res ){
      if( err ) {
        done( err, null );
      } else {
        if( res.length ){
  ...
  function(req, login, password, done) {
    ...
    if( req.session.loggedIn ){
      ...
      userStrategies.apiGetQuery( { conditions: { strategyId: 'local', field1: login.toLowerCase() } }, function( err, res ){
        if( err ) {
          done( err, null );
        } else {
          if( ! res.length ){
        ...
        if( err ) {
          ...
          if( res.length ){
              ...
              if( err ){
                ...
                userStrategies.apiPost( { userId: user.id, strategyId: 'local', field1: login.toLowerCase(), field3: password }, { killComet: true }, function( err, res ){
                  if( err ) {
                    done( err, null );
                  } else {
