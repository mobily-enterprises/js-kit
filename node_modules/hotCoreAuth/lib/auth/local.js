...
var exports = module.exports = function( strategyConfig, makeResponder, AuthStrategies, Users, UserStrategies, UserLogins ) {
  ...
  function(req, login, password, done) {
    ...
    UserStrategies.GetQuery( { filters: { field1: login.toLowerCase(), field2: password } }, function( err, res, idProperty ){
      if( err ) {
        done( err, null );
      } else {
        if( res.length ){
        ...
        if( err ) {
          ...
          if( res.length ){
              ...
              if( err ){
                ...
                UserStrategies.Post( { userId: user[ idProperty ], strategyId: 'local', field1: login.toLowerCase(), field2: password }, function( err, res, idProperty ){
                  if( err ) {
                    done( err, null );
                  } else {
