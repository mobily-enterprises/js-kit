...
var exports = module.exports = function( app, strategyConfig, makeResponder, AuthStrategies, Users, UserStrategies, UserLogins ) {
  ...
  function(req, login, password, done) {
    ...
    UserStrategies.GetQuery( { filters: { userId: req.session.userId, strategyId: 'local' } }, function( err, res ){
      if( err ) {
        done( err, null );
      } else {
        if( res.length ){
          ...
          var foundStrategyId = res[0]._id;
        }

        UserStrategies.GetQuery( { filters: { field1: login.toLowerCase() } }, function( err, res ){
          ...
          if( err ) {
            ...
            if( res.length > 0 ){
              editingSelf = ( itsEdit == true && res[0]._id.toString() == foundStrategyId.toString()  );
            }
            ...
            if( res.length > 0 && !editingSelf ){
              ...
              if( itsEdit ){
                ...
                UserStrategies.Put( foundStrategyId, { userId: req.session.userId, strategyId: 'local', field1: login.toLowerCase(), field3: password }, function( err, res ){
                  if( err ) {
                    done( err, null );
                  } else {
                    complete( res._id );
              ...
              // It's a new entry: add a new record
                ...
                UserStrategies.Post( { userId: req.session.userId, strategyId: 'local', field1: login.toLowerCase(), field3: password }, function( err, res ){
                  if( err ) {
                    done( err, null );
                  } else {
                    complete( res._id );
  ...
  function(req, login, password, done) {
    ...
    UserStrategies.GetQuery( { filters: { field1: login.toLowerCase(), field3: password } }, function( err, res ){
      if( err ) {
        done( err, null );
      } else {
        if( res.length ){
  ...
  function(req, login, password, done) {
    ...
    if( req.session.loggedIn ){
      ...
      UserStrategies.GetQuery( { filters: { strategyId: 'local', field1: login.toLowerCase() } }, function( err, res ){
        if( err ) {
          done( err, null );
        } else {
          if( ! res.length ){
      ...
      UserStrategies.GetQuery( { filters: { strategyId: 'local', field1: login.toLowerCase() } }, function( err, res ){
        if( err ) {
          ...
          if( res.length ){
              ...
              Users.Post( { }, function( err, user ){

              if( err ){
                done( err, null );
              } else {

                UserStrategies.Post( { userId: user._id, strategyId: 'local', field1: login.toLowerCase(), field3: password }, function( err, res ){
                  if( err ) {
                    done( err, null );
                  } else {
