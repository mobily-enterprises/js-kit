...
exports.strategyRoutesMaker = function( app, strategyConfig, done ) {
  ...
  hotCoreStoreRegistry.getAllStores( function( err, stores ){
    ...
    function checkFacebookToken( token, cb ){
      ...
      var params = { qs: { access_token: token }, json: true };

      request.get( 'https://graph.facebook.com/me', params, function( err, response, profile ){
        if( err ) return cb( err );

        request.get( 'https://graph.facebook.com/app', params, function( err, response, appInfo ){
          if( err ) return cb( err );
    ...
    app.post( hotplate.prefix( '/auth/resume/facebook/postcheck' ), function( req, res, err ){
      ...
      checkFacebookToken( req.body.accessToken, function( err, profile ){
        ...
        passportResume( req, body.accessToken, req.body.requestTonken, profile, makeResponder( req, res, next, 'facebook', 'resume' ) );
      });
    });
