...
exports = module.exports = function( app, strategyConfig,  makeResponder, authStrategies, users, userStrategies, userLogins ) {
  ...
  function(req, accessToken, refreshToken, profile, done) {
    ...
    if( ! req.session.loggedIn ){
      ...
      userStrategies.apiGetQuery( { filters: { userId: req.session.userId, strategyId: 'facebook' } }, function( err, res ){
        if( err ) {
          done( err, null );
        } else {
          if( res.length ){
            ...
            // Check that "facebook" isn't already there
            userStrategies.apiGetQuery( { filters: { field1: profile.id } }, function( err, res ){
              if( err ) {
                done( err, null );
              } else {
                if( res.length ){
  ...
  function(req, accessToken, refreshToken, profile, done) {
    ...
    if( req.session.loggedIn ){
      ...
      userStrategies.apiGetQuery( { filters: { strategyId: 'facebook', field1: profile.id } }, function( err, res ){
        if( err ) {
          done( err, null );
        } else {
