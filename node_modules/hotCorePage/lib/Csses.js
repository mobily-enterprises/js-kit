"use strict";
/*!
 * Module dependencies.
 */

var dummy
  , hotplate = require('hotplate')
  , path = require('path')
;

/**
 * Csses constructor.
 *
 * An object to which you can add CSS files to, and render them
 *
 * @api public
 */

function Csses(){
  this.csses = [];
  this.Csses = Csses;
}

/**
 * The exports object is the object's constructor
 *
 * @api public
 */

module.exports = exports = Csses;


/**
 * Css rendering function.
 *
 * This will return a string which can be dropped into the HEAD of an HTML page
 *
 * @api public
 */

Csses.prototype.render = function(){
  var r = '';
  this.csses.forEach( function(css){
    r += '<link href="' + path.join( hotplate.config.get('hotplate.staticUrlPath'), css.module, css.fileLocation) + '" media="screen" rel="stylesheet" type="text/css" />' + "\n";
  });
  return r;
}

/**
 * Add a CSS file
 *
 * @param {String} The module's name
 * @param {String} The file's full path
 * @return {Csses} this
 * @api public
 */

Csses.prototype.add = function(module, fileLocation ){
  this.csses.push( { module:module, fileLocation: fileLocation } );
  return this;
}


Csses.prototype.concat = function( csses ){
  for( var i = 0, l = csses.csses.length; i < l; i ++ ){
    this.add( csses.csses[ i ].module, csses.csses[ i ].fileLocation );
  }
}


