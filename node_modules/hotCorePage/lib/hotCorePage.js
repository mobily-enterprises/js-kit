...
function replacePageElements( template, elements){
  ...
  TYPES.forEach( function( type ){
    if( elements[ type ] ){
      var t = elements[ type ].render().replace(/\$/g, '$$$$');
...
var acquireElements = exports.acquireElements = function( parameterElements, req, pageName, done ){
  ...
  elements.jses = new hotCorePage.Jses;
  elements.csses = new hotCorePage.Csses;
  elements.vars  = new hotCorePage.Vars;
  elements.headLines = new hotCorePage.HeadLines;
  elements.bodyLines = new hotCorePage.BodyLines;
  elements.titleWords = new hotCorePage.TitleWords;
  ...
  hotplate.hotEvents.emit( 'pageElements', function( err, results ){
    if( err ){
      ...
      hotplate.hotEvents.emit('pageElementsPerPage', req, pageName, function( err, results ){
        if( err ){
          done( err );
        } else {
