...
exports.makeTabIdHashForMultihome = function( userId, tabId, message, done ){
  ...
  hotCoreStoreRegistry.getAllStores( function( err, allStores ){
    if( err ){
      ...
      if( typeof(  allStores.stores[ message.storeName ] ) === 'undefined' ){
      ...
      allStores.stores.Tabs.Store.GetQuery( { }, function( err, tabs ){
        if( err ){
          ...
          var hotGlobalBroadcast = allStores.stores[ message.storeName ].storeObject.hotGlobalBroadcast;
          ...
          // to that specific workspaceId
            ...
            allStores.stores.WorkspacesUsersBase.Store.GetQuery( { filters: { workspaceId: message.object.workspaceId } }, function( err, workspacesUsers ){
          ...
          // who are in the same workspace as userId
            ...
            allStores.stores.WorkspacesUsersBase.Store.GetQuery( { filters: { userId: userId } }, function( err, workspacesUsers ){
              ...
              workspacesUsers.forEach( function( w ){
                ...
                // Get a list of users for that workspace
                allStores.stores.WorkspacesUsersBase.Store.GetQuery( { filters: { workspaceId: w.workspaceId } }, function( err, workspacesUsers ){
...
var userInWorkspace = exports.userInWorkspace = function( userId, workspaceId, done ){
  ...
  stores.WorkspacesUsersBase.GetQuery( { filters: { userId: userId, workspaceId: workspaceId } }, function( err, docs ){
    if( err ){
      done( err );
    } else {
