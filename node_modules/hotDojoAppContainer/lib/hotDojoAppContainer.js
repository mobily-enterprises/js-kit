  ...
  if( req.params.workspaceId ){
    ...
    hotCoreStoreRegistry.getAllStores( function( err, allStores ){
      if( err ){
        ...
        allStores.workspaces.apiGet( req.params.workspaceId, function( err, record ){
          if( err ){
            next( err );
          }  else {

            // Check that the user is allowed on the workspace
            allStores.workspacesUsers.apiGetQuery( { conditions: { workspaceId: req.params.workspaceId, userId: req.session.userId } }, function( err, wsRecords ){
              if( err ){
                next( err );
              } else {
