...
hotplate.hotEvents.on( 'setRoutes', function( done ){
   ...
   // Pages
  hotplate.app.get('/pages/welcome' , pageWelcome);
  hotplate.app.get('/pages/pick' , pagePick );
    ...
    var allStores = hotCoreStoreRegistry.getAllStores( function( err, allStores ){
      ...
      allStores.stores.userWorkspaces.Store.GetQuery( { filters: { userId: req.session.userId  }  }, function( err, records ){
        if( err ){
          ...
          if( records.length !== 1 || ! hotplate.get('hotCoreMultiHome').escapePick ){
            returnThePage();

          // OK, escape successful! Get the application URL and jump there
          } else {
            var applicationURL = hotplate.get('hotCoreMultiHome').multiHomeURL;
