...
var pageWelcome = function(req, res, next){
  ...
  if(! req.session.loggedIn){
    ...
    var extraTitleWords = new hotCorePage.TitleWords();
    var extraBodyLines = new hotCorePage.BodyLines();

    extraTitleWords.add( 'hotDojoAuth', 'QUADRUPLE' );
    extraBodyLines.add( 'hotDojoAuth', '<div data-dojo-type="hotplate/hotDojoAuth/Welcome" id="welcome"></div>' );
    ...
    hotCorePage.processPageTemplate(
      {
        ...
        csses: extraCsses,
        vars: extraVars,
        titleWords: extraTitleWords,
        bodyLines: extraBodyLines,
      },
      req,
      'hotDojoAuth/Welcome',
      function( err, result ){
...
var pagePick = function(req, res, next){
  ...
  if( req.session.loggedIn){
    ...
    var allStores = hotCoreStoreRegistry.getAllStores( function( err, allStores ){
      ...
      allStores.stores.UsersWorkspaces.Store.GetQuery( { filters: { userId: req.session.userId  }  }, function( err, records ){
        if( err ){
          next( err );
        }  else {
      ...
      function returnThePage(){
        ...
        extraBodyLines.add( 'hotDojoAuth', '<div data-dojo-type="hotplate/hotDojoAuth/Pick" id="pick">' );
