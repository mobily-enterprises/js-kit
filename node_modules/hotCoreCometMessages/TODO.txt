 ...
 * Make a store called "tabs"
 * Make it so that registering means posting to the tabs store
 * Do NOT add a public URL for the store so that it's not online by default (only blind post avaiable by default)
 * Only user/tab pairs are registered, NO workspaceId
 * Broadcasting:
