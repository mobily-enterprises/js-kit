...
define([
 ...
 , globals
  ...
  Object.keys( vars.hotCoreStoreConfig.configStores ).forEach( function( storeName ){
    ...
    // Get the store's object. Note: a REQUIREMENT is that config objects are first-level, no string replacement possible
    // as this generic "get me the store" function wouldn't know what to replace things with.
    // Note that config stores are always dealt with with GET and PUT from the client side, using that record's ID

    // var store = stores( storeName, { workspaceId: globals.workspaceId, userId: globals.userId } ); // OLD AND WRONG
    ...
    topic.subscribe('storeRecordUpdate', function( from, message){
      ...
      if( message.storeName == storeName && message.object[ store.idProperty ] == r[ message.storeName ][store.idProperty] ){
