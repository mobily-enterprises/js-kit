          ...
          if( tab.length != 1 && tab.length != 0 ){
            ...
            exports.Tabs.Post( { lastSync: new Date(), userId: self._req.session.userId }, {}, function( err, tab ){
              if( err ){
                ...
                hotCoreStoreConfig.getConfig( self._req.headers[ 'x-hotplate-workspaceid' ], self._req.session.userId, function( err, configStoreRecords ){
                  if( err ){
                    done( err );
                  } else {
                    done( null, { messages: [ { fromUserId: self._req.session.userId, message: { type: 'resetStores', tabId: tab.tabId, configStoreRecords: configStoreRecords } } ] } );
                  };
                });
