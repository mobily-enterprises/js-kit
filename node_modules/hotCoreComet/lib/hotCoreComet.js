...
hotplate.hotEvents.on( 'stores', 'hotCoreComet', function( done ){
  ...
  stores.TabMessages = declare( PermStore, {
  ...
  stores.TabMessageDispatcher = declare( Store, {
    ...
    driverAllDbFetch: function( params, body, options, done ){
      ...
      stores.Tabs.GetQuery( { filters: { tabId: params.tabId, fromLastSync: new Date() - TABLIFESPAN } }, function( err, tab ){
        if( err ){
          done( err );
        } else {
          if( tab.length != 1 && tab.length != 0 ){
            ...
            hotCoreRegistry.getAllStores( function( err, storesData ){
              if( err ){
                done( err );
              } else {
                storesData.stores.UserWorkspaces.GetQuery( { filters: { userId: self._req.session.userId, workspaceId: headersWorkspaceId } }, function( err, uwDocs){
                  if( err ){
                    done( err );
                  } else {
                    if( uwDocs.length == 0 ){
  ...
  // done( null, stores );
  done( null,  [ s.Tabs, s.TabMessages, s.TabMessageDispatcher ] );

})


hotplate.hotEvents.on( 'run', function( done ){
  ...
  hotCoreStoreRegistry.getAllStores( function( err, storesData ){
    if( err ){
      ...
      setInterval( function(){
        ...
        storesData.stores.Tabs.MassDelete( { filters: { toLastSync: new Date() - TABLIFESPAN } }, function ( err ){
          //TODO: Log error
          storesData.stores.TabMessages.MassDelete( { filters: { toAdded: new Date() - TABLIFESPAN } }, function ( err ){
            // TODO: Log error
            cleaningUp = false;
          });
        });
    ...
    if( err ){
      ...
      makeTabIdHash( userId, tabId, message, function( err, tabIdHash ){
        ...
        for( tabId in tabIdHash ){
          storesData.stores.TabMessages.Post( { tabId: tabId, fromUserId: userId, message: message }, function( err ) {
            if( err ){
              // TODO: log err
            }
          } );
...
function allTabIdHash( userId, tabId, message, done ){
  ...
  hotCoreStoreRegistry.getAllStores( function( err, storesData ){
    if( err ){
      ...
      storesData.store.Tabs.GetQuery( { filters: { fromLastSync: new Date() - TABLIFESPAN  }  }, function( err, docs ){
        if( err ){
          done( err );
        } else {
