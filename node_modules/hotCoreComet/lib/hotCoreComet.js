   ...
   hotCoreJsonRestStores.get( function( err, s ){
    ...
    var TabMessageDispatcher = declare( JsonRestStores, {
      ...
      implementFetchOne: function( request, done ){
        ...
        debug("Looking for tab %s owned by user %s, x-workspaceId is %s, expiration is %s, now is %s", request.params.tabId, request._req.session.userId, headersWorkspaceId, new Date( new Date() - TABLIFESPAN ), new Date() );
        ...
        stores.tabs.dbLayer.select( { conditions: {
          ...
          if( tabs.length == 0 ){
            ...
            hotCoreStoreRegistry.getAllStores( function( err, storesData ){
              ...
              storesData.usersWorkspaces.dbLayer.selectByHash( { conditions: { userId: request._req.session.userId, workspaceId: headersWorkspaceId } }, { DEBUGME: true }, function( err, uwDocs){
                if( err ) return done( err );
