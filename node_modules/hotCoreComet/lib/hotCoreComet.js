      ...
      stores.Tabs.GetQuery( { filters: { tabId: params.tabId, fromLastSync: new Date() - TABLIFESPAN } }, function( err, tab ){
        if( err ){
          ...
          if( tab.length != 1 && tab.length != 0 ){
            ...
            hotCoreMultiHome.getStores( function( err, hotCoreMultiHomeStores ){
              if( err ){
                done( err );
              } else {
      
                hotCoreMultiHomeStores.UserWorkspaces.GetQuery( { filters: { userId: self._req.session.userId, workspaceId: headersWorkspaceId } }, function( err, uwDocs){
                  if( err ){
                    done( err );
                  } else {
                    if( uwDocs.length == 0 ){
...
hooks.stores = function( done ){
  ...
  getStores( function( err, s){
    if( err ){
      done( err );
    } else {

      done( null,  {
        moduleName: 'hotCoreComet', result: [ s.Tabs, s.TabMessages, s.TabMessageDispatcher ]
      });
  ...
  getStores( function( err, stores ){
    if( err ){
      ...
      setInterval( function(){
        ...
        stores.Tabs.MassDelete( { filters: { toLastSync: new Date() - TABLIFESPAN } }, function ( err ){
          //TODO: Log error
          stores.TabMessages.MassDelete( { filters: { toAdded: new Date() - TABLIFESPAN } }, function ( err ){
            // TODO: Log error
            cleaningUp = false;
          });
        });
    ...
    if( err ){
      ...
      makeTabIdHash( userId, tabId, message, function( err, tabIdHash ){
        ...
        for( tabId in tabIdHash ){
          stores.TabMessages.Post( { tabId: tabId, fromUserId: userId, message: message }, function( err ) {
            if( err ){
              // TODO: log err
            }
          } );
...
function allTabIdHash( userId, tabId, message, done ){
  ...
  getStores( function( err, stores ){
    if( err ){
      ...
      stores.Tabs.GetQuery( { filters: { fromLastSync: new Date() - TABLIFESPAN  }  }, function( err, docs ){
        if( err ){
          done( err );
        } else {
