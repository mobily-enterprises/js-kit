  ...
  // it doesn't mixin with hotJsonRestStores (which would do Comet event emission etc.)
   ...
   hotCoreJsonRestStores.get( function( err, s ){
    ...
    var Tabs = declare( BasicDbStore, {
      ...
      // Note: this is only ever used internally, since handleGetQuery is false
      onlineSearchSchema: new BasicSchema({
      //  id          :  { type: 'id' },
      //  userId      :  { type: 'id' },
        //fromLastSync:  { type: 'date' },
        //toLastSync:    { type: 'date' },
      }),
      ...
      queryConditions: {
        name: 'and',
        args: [
          { name: 'gte', args: [ 'lastSync', '#fromLastSync#' ] },
          { name: 'lte', args: [ 'lastSync', '#toLastSync#' ] },
          { name: 'eq',  args: [ 'id', '#id#'] },
          { name: 'eq',  args: [ 'userId', '#userId#'] },
        ]
      },
      */
      ...
      afterEverything: function( request, method, p, cb ){
        ...
        stores.tabMessages.dbLayer.delete( { name: 'eq', args: [ 'tabId', request.body.tabId ] }, cb );
    ...
    var TabMessages = declare( BasicDbStore, {
      ...
      queryConditions: {
        name: 'and',
        args: [
          { name: 'gte', args: [ 'added', '#fromAdded#' ] },
          { name: 'lte', args: [ 'added', '#toAdded#' ] } ,
          { name: 'eq',  args: [ 'id', '#id#' ] } ,
          { name: 'eq',  args: [ 'tabId', '#tabId#' ] } ,
        ]
      },
    ...
    var TabMessageDispatcher = declare( JsonRestStores, {
      ...
      checkPermissions: function( request, method, p, done ){
        ...
        //stores.tabs.apiGetQuery( { conditions: { fromLastSync: new Date() - TABLIFESPAN, id: request.params.tabId } }, function( err, tab ){
        stores.tabs.dbLayer.select( { conditions: {
          ...
          if( tabs.length == 0 ){
            ...
            done( null, tabs[0].userId.toString() === request._req.session.userId.toString() );
          }
      ...
      implementFetchOne: function( request, done ){
        ...
        stores.tabs.dbLayer.select( { conditions: {
          ...
          args: [
            ...
            {
              name: 'eq',
              args: [ 'userId', request._req.session.userId ]
            },
            ...
            if( tabs.length == 0 ){
              ...
              hotCoreStoreRegistry.getAllStores( function( err, storesData ){
                if( err ){
                  ...
                  storesData.usersWorkspaces.dbLayer.selectByHash( { conditions: { userId: request._req.session.userId, workspaceId: headersWorkspaceId } }, function( err, uwDocs){
                    if( err ){
                      done( err );
                    } else {
                      if( uwDocs.length == 0 ){
                        ...
                        stores.tabs.dbLayer.insert( { userId: request._req.session.userId }, { returnRecord: true }, function( err, tab ){
                          if( err ){
                             done( err );
                          } else {
              ...
              stores.tabs.dbLayer.updateById( tab.id, { lastSync: tab.lastSync }, function( err, tab ){
                if( err ){
                  ...
                  stores.tabMessages.dbLayer.select( { sort: { added: 1 }, conditions: { 
                    name: 'and',
                    args: [
                      {
                        name: 'gte',
                  ...
                  } }, { delete: true }, function( err, tabMessages ){
  ...
  hotCoreStoreRegistry.getAllStores( function( err, storesData ){
    if( err ){
      ...
      setInterval( function(){
        ...
        storesData.tabs.dbLayer.delete( { name: 'lte', args: [ 'lastSync', new Date( new Date() - TABLIFESPAN ) ] }, { multi: true }, function( err, howMany ){
          // debug( 'Tabs: error and howMany: ', err, howMany );
          //TODO: Log error

          storesData.tabMessages.dbLayer.delete( { name: 'lte', args: [ 'added', new Date( new Date() - TABLIFESPAN ) ] }, { multi: true }, function( err, howMany ){
      ...
      makeTabIdHash( userId, tabId, message, function( err, tabIdHash ){
        ...
        async.each( 
          ...
          function( tabId, cb){
            ...
            storesData.tabMessages.dbLayer.insert( obj, { returnRecord: true }, cb );
          },
