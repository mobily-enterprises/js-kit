  ...
  // it doesn't mixin with hotJsonRestStores (which would do Comet event emission etc.)
   ...
   hotCoreJsonRestStores.get( function( err, s ){
    ...
    var TabMessageDispatcher = declare( JsonRestStores, {
      ...
      handleGet: true,
      checkPermissionsGet: function( request, doc, fullDoc, done ){
      ...
      execAllDbFetch: function( request, done ){
        ...
        if( ! request._req.session.loggedIn || ! request._req.session.userId ){
          hotplate.log("A non-logged in user tried to fetch tabId %s for workspaceId %s, denying...", request.params.tabId, headersWorkspaceId );
          return done( new self.UnauthorizedError() );
        }
  
        hotplate.log("Looking for tab %s owned by user %s, x-workspaceId is %s", request.params.tabId, request._req.session.userId, headersWorkspaceId );
                  ...
                  storesData.usersWorkspaces.apiGetQuery( { filters: { userId: request._req.session.userId, workspaceId: headersWorkspaceId } }, function( err, uwDocs){
                    if( err ){
                      ...
                      if( uwDocs.length == 0 ){
                        ...
                        hotplate.log( request._req.session.userId );
