...
exports = module.exports = {
  ...
  setRoutes: function( stores, app, configArray, done ){
    ...
    var updateUrl = hotplate.config.get('hotCoreTransport.activeTransports.sms-plivo.updateUrl');
    if( updateUrl ){

      debug("Update URL for Plivio is: " + url.parse( updateUrl ).pathname  );
      // Set route for incoming status changes
      app.post( url.parse( updateUrl ).pathname , function( req, res, next ){
      //app.get( '/app/incomingsms/plivo-update', function( req, res, next ){
        ...
        if (body.ParentMessageUUID != '-' && body.ParentMessageUUID !== body.MessageUUID ){
          ...
          logLine( config, null, 1, "Status update for subpart " + body.MessageUUID + " received; only status update of main part is considered. Status received however was: " + body.Status );
          return next( null );
        }

        // Looking up the message
        stores.messagesTo.apiGetQuery( { filters: { foreignId: body.MessageUUID } }, function( err, there ){
          if( err ){
            logLine( config, null, 3, "Could not update the status of message " + body.MessageUUID + ", received: ", body.Status, err );
            return next( err );
          }
          if( there.length === 0 ){
            logLine( config, null, 3, "Attempted to update the status of non-existent message " + body.MessageUUID + ", received: " + body.Status, new Error() );
            return next( null );
          }
          if( there.length > 1 ){
            logLine( config, null, 3, "Multiple messages matching this ID found for message  " + body.MessageUUID + ", received: " + body.Status );
            return done( null );
          }
          ...
          if( messageTo.status === 'delivered' ){
            ...
            logLine( config, messageTo, 1, "Request change of status for message " + body.MessageUUID + ". Received status: " + body.Status + " which would translates into " + newStatus + ". However, original status message was " + messageTo.status + " which means that the status will NOT be changed." );
          ...
          logLine( config, messageTo, 1, "Changing status for message " + body.MessageUUID + ". Received status: " + body.Status + " which translates into " + newStatus );
          changeMessageStatus( 'sms-plivo', config, messageTo, false, newStatus, function( err ){
            if( err ){
              logLine( config, messageTo, 3, "Could not update the status of message " + body.MessageUUID + ", received: " + body.Status + " which translates into " + newStatus , err );
              return next( err );
            }

            logLine( config, messageTo, 1, "Status of message " + body.MessageUUID + ", changed, received: " + body.Status + " which translates into " + newStatus , err );
    ...
    var route = hotplate.config.get('hotCoreTransport.activeTransports.sms-plivo.smsRoute', '/app/incomingsms/plivo' );
      ...
      stores.messages.apiPost( { 
        ...
        stores.messagesTo.apiPost( {
          ...
          stores.messages.apiGet( messageRecord.id, function( err, fullMessageRecord ){
            ...
            hotplate.hotEvents.emit( 'transportMessageFetched', 'sms-plivo', config, fullMessageRecord, messageToRecord, function( err ) { 
              if( err ){ 

                logLine( config, null, 3, "Error after message " + fullMessageRecord.foreignId + "processed", err );  
                deleteUnfinishedMessage( config, messageRecord.id );
    ...
    request.post(
      ...
      function( err, response, body ){
        ...
        if( response.statusCode !== 202 ){
          logLine( config, messageTo, 1, "Attempt to send SMS failed server responded with status " + response.statusCode );
          changeMessageStatus( 'sms-plivo', config, messageTo, true, 'todeliver', function(){} );
          return; 
        }
        ...
        stores.messagesTo.apiPut( null, messageTo, function( err ){
          ...
          if( err ){
            logLine( config, messageTo, 3, "Failed to update local message with foreign message ID; status updates won't be registered, assuming the message will be delivered", err );
          ...
          logLine( config, messageTo, 1, "SMS handed over to gateway, waiting for status updates" );
        })
      }
    );
