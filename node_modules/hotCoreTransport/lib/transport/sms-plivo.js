  ...
  setRoutes: function( stores, app, configArray, done ){
    ...
    app.all(route, function( req, res, next ){
      ...
      stores.messages.dbLayer.insert( { 
        ...
        bodyText   : body.Text,
        from       : body.From,
      }, { returnRecord: true }, function( err, messageRecord ){
        if( err ){
        ...
        }, { returnRecord: true }, function( err, messageToRecord ){

          // If there was DB error, clean up and return error
          if( err ){
            logLine( config, null, null, 3, "Error attaching recipient " + body.To + " to message " + fullMessage.foreignId, err );
  ...
  changeMessageStatus( 'sms-plivo', config, messageTo, false, 'delivering', function( err ){
    ...
    request.post(
      ...
      function( err, response, body ){
        ...
        stores.messagesTo.updateById( messageTo.id, { foreignId: foreignId }, function( err ){
