...
TODO:
* Make sure email is sent as TXT and HTML by client
* Make sure email is displayed properly whether it's HTML or text
* Implement SMS with Plivo
...
exports = module.exports = {
  ...
  sendMessage: function( stores, messageTo, config ){
    sendMessage( stores, messageTo, config );
  },
...
function sendMessage( stores, messageTo, config ){
  ...
  changeMessageStatus( config, messageTo, true, 'delivering', function( err ){
    ...
    var c = transporter.sendMail({
      ...
      // propagated up
      if( err ){
        logLine( stores, config, messageTo, 1, "Could not send email", err );
        changeMessageStatus( config, messageTo, true, 'todeliver', function(){} );
        return;
      }

      // All good: change the status to delivered, log sending
      logLine( stores, config, messageTo, 1, "Email delivered successfully!" );
...
function poll( stores, config, force ){
