...
function _sendMessage( stores, messageTo, config ){
  ...
  changeMessageStatus( 'email-default', config, messageTo, false, 'delivering', function( err ){
    ...
    if( messageId.bodyText ) m.text = messageId.bodyText;
    
    var c = transporter.sendMail( m, function( err, info ){

      // There was an error: log it
      if( err ){
        logLine( config, messageTo, 2, "Could not send email", err );
        changeMessageStatus( 'email-default', config, messageTo, true, 'todeliver', function(){} );
        return;
      }

      // All good: change the status to delivered, log sending
      logLine( config, messageTo, 1, "Email delivered successfully!" );
      ...
      if( info.messageId ){
        messageTo.foreignId = info.messageId;
      }
      messageTo.foreignData = { deliveryInfo: info };
      //
      stores.messagesTo.apiPut( null, messageTo, function( err ){
        if( err ){
          logLine( config, messageTo, 3, "Failed to set remote message ID and response; doing so is not critical ", err ); 
        }
      });
    });
      ...
      async.eachSeries(
        ...
        function( partID, cb ){
          ...
          fetchMessagePart( imap, fullImapMessage.uid, partID.partID, function( err, part ){
            ...
            fullImapMessage[ partID.type ] = part.body;
            cb( null );
          })
        },
        function( err ){
        ...
        async.eachSeries(
          ...
          function( uid, uidsIterationCb ){
            ...
            fetchFullMessage( config, imap, uid, { bodies: 'HEADER.FIELDS (FROM TO SUBJECT DATE )', struct: true }, function( err, fullImapMessage){
              ...
              stores.messagesTo.apiGetQuery( { filters: { foreignId: fullImapMessage.uid } }, function( err, there ){
                if( err ){
                  logLine( config, null, 3, "Could not check if message " + fullImapMessage.uid + " was already in the database.", err );
                  return uidsIterationCb( null );
                }
                if( there.length > 0 ){
                  logLine( config, null, 1, "Message " + fullImapMessage.uid + " was already in the database, skipping" ); 
                ...
                var m = { 
                  ...
                  bodyText   : fullImapMessage.body,
                  bodyHtml   : fullImapMessage.bodyHtml,
                  from       : fullImapMessage.headers.from[ 0 ],
                };

                hotplate.hotEvents.emit( 'transportManipulateMessageBeforeSave', 'email-default', m, config, function( err ){

                  if( err ){
                    logLine( config, null, 3, "Error in manipulation (m) before storing message " + fullImapMessage.uid, err ); 
                    return uidsIterationCb( null );
                  }

                  stores.messages.apiPost( m, function( err, messageRecord ){

                    if( err ){
                      logLine( config, null, 3, "Error storing message " + fullImapMessage.uid, err ); 
                      return uidsIterationCb( null );
                    }
                    ...
                    hotplate.hotEvents.emit( 'transportManipulateMessageToBeforeSave', 'email-default', mt, config, function( err ){

                      if( err ){
                        logLine( config, null, 3, "Error in manipulation (mt) before storing message " + fullImapMessage.uid, err ); 
                      ...
                      stores.messagesTo.apiPost( mt, function( err, messageToRecord ){
                        if( err ){
                          logLine( config, null, 3, "Error attaching recipient " + config.systemEmail + " to message " + fullImapMessage.uid, err );
                        ...
                        async.eachSeries(
                          ...
                          function( attachment, attachmentIterationCb ){
                            ...
                            stores.messagesAttachments.apiPost(a, function( err, partRecord ){
                              if( err ){
                                logLine( config, null, 3, "Error attaching attachment " + attachmentPart.partID + " to message " + fullImapMessage.uid, err ); 
                          ...
                          function( err ){
                            ...
                            stores.messages.apiGet( messageRecord.id, function( err, fullMessageRecord ){
                              ...
                              if( err ){
                                logLine( config, null, 3, "Error re-reading the message once sent: " + fullImapMessage.uid + " to broadast it", err ); 
                              ...
                              hotplate.hotEvents.emit( 'transportMessageFetched', 'email-default', config, fullMessageRecord, messageToRecord, function( err ) { 

                                if( err ){ 
                                  logLine( config, null, 3, "Error after message " + fullImapMessage.uid + " processed", err ); 
