  ...
  hotCoreJsonRestStores.get( function( err, s ){
    ...
    var Messages = declare( [ BasicDbStore ],  {
      ...
      schema: new HotSchema({
        ...
        body        : { type: 'string', searchable: true, required: true,  notEmpty: false, trim: 256000 },
        bodyHtml    : { type: 'string', searchable: true, required: true,  notEmpty: false, trim: 256000 },
...
var startPolling = exports.startPolling = function( transport, stores, config, done ){
  require('./transport/' + transport + '.js' ).startPolling( stores, config, done );
}
...
function startTransportCron(){
  ...
  var inCycle = false;
  var inOutgoingCycle = false;


  hotCoreStoreRegistry.getAllStores( function( err, storeRegistry){
    if( err ) {
      // TODO: logger.log needs to be here
      hotplate.critical("Error getting all stores in startTransportCron:", err );
      return;
    };
    ...
    setInterval( function(){
      ...
      debug("Running cron function to fetch messages")

      // Do not run cron twice
      if( inOutgoingCycle) {
        debug("Cron function to fetch messages was quit as it was still running");
        return;
      }
      inOutgoingCycle = true;
        ...
        function( transport, cb ){
          ...
          getAllTransportConfig( transport, function( err, configArray ){
            ...
            configArray.forEach( function( config ){
              ...
              startPolling( transport, stores, config,  function( err ){
                if( err ){
                  logger.log( { error: err, system: true, logLevel: 3, message: "Error while running startPolling() for transport " + transport, data: { config: config } } );
    ...
    setInterval( function(){
      ...
      stores.messagesTo.dbLayer.select( {conditions: { and: [ /*{ field: 'failedAttempts', 'type': 'lt', value: 50 }, */ { field: 'status', type: 'is', value: 'todeliver' } ]  } }, { useCursor: true , delete: false, children: true }, function( err, cursor, total, grandTotal ){
        if( err ) {
          ...
          // TODO: logger.log needs to be here
          hotplate.critical("Error getting cursor from messagesTo:", err );
          return;
        }; 
          ...
          function( callback ){
            cursor.next( function( err, i ){
              ...
              getTransportConfig( messageTo, function( err, transportConfg ){
                ...
                // Undeliverable messages mustn't do that.
...
exports.transportLayerFunctions.logLine = function( config, messageTo, logLevel, line, errInLogline ){
  ...
  if( errInLogline ){
    ...
    // a hotplate. critical() ( which will display on the screen)
      ...
      logger.log( { error: errInLogline, system: true, logLevel: logLevel, message: line, data: { config: config, messageTo: messageTo } } );
    }
  }
