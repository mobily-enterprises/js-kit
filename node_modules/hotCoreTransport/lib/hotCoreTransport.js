...
hotplate.hotEvents.on( 'setRoutes', 'hotCoreTransport', function( app, done ){
  async.eachSeries(
    ...
    function( transport, cb ){
      ...
      transportModule.setRoutes( stores, app, getAllTransportConfig, cb );
    },
    function( err ){
...
hotplate.hotEvents.on( 'run', 'hotCoreTransport', function( done ){
  ...
  async.eachSeries(
    ...
    function( transport, cb ){
      var transportModule = require('./transport/' + transport + '.js' );
      // Run setRoutes for relevant module
      transportModule.run( stores, getAllTransportConfig, cb );
    },
    function( err ){
      if( err ) return done( err );  
...
function dealWithMessage(  stores, messageTo, config, done ){
  ...
  stores.messagesTo.dbLayer.update( { conditions: { and: [ { field: 'id', type: 'is', value: messageTo.id } ]  } }, { status: 'delivering' }, {}, function( err ){
    ...
    messageTo.status = 'delivering';
    transportModule.sendMessage( stores, messageTo, config.config, done );
...
function startTransportCron(){
  ...
  setInterval( function(){
    ...
    if( inCycle) {
      console.log("*********************************************************");
      console.log("**********QUIT AS STILL RUNNING**************************");
      console.log("*********************************************************");
      return;
    }
    inCycle = true;
    ...
    hotCoreStoreRegistry.getAllStores( function( err, storeRegistry){
      if( err ) {
        inCycle = false; 
        hotplate.log("Error getting all stores in startTransportCron:", err );
        return;
      };
            ...
            cursor.next( function( err, i ){
              ...
              if( messageTo !== null ){
                ...
                getTransportConfig( messageTo, function( err, config ){
                  ...
                  dealWithMessage( stores, messageTo, config, function( err){
                    if( err ) { return callback( err ); };
  ...
  hotCoreJsonRestStores.get( function( err, s ){
    ...
    var Messages = declare( [ BasicDbStore ],  {
      ...
      schema: new HotSchema({
        ...
        foreignData : { type: 'string', searchable: true, required: false,  notEmpty: true, trim: 256000 }, 
    ...
    var MessagesAttachments = declare( [ BasicDbStore ],  {
      ...
      schema: new BasicSchema({
        ...
        attachment     : { type: 'data', searchable: false, required: true, notEmpty: true },        
      }),
    ...
    var MessagesDeliveryLog = declare( [ BasicDbStore ],  {
      ...
      schema: new BasicSchema({
        ...
        level         : { type: 'number' },
        line          : { type: 'string', searchable: true, required: true, notEmpty: true, trim: 255 },        
      }),
