        ...
        async.doWhilst(
          ...
          function( callback ){
            ...
            cursor.next( function( err, messageTo ){
              ...
              console.log("MESSAGETO 1 HERE IS:", messageTo );
              ...
              console.log("MESSAGETO 2 HERE IS:", messageTo );
              ...
              getTransportConfig( messageTo, function( err, transportConfig ){
                if( err ){
                  ...
                  logLine( null, messageTo, 3, "Error while getting transport configuration", err );
                  return;
                }
                ...
                console.log("MESSAGETO 3 HERE IS:", messageTo, transportConfig );
                ...
                hotplate.hotEvents.emit( 'transportManipulateBeforeSend', transportConfig.transport, messageTo, transportConfig.config, function( err ){
                  ...
                  console.log("MessageTO AFTER CURE: ", messageTo );

                  // Could not pre-process messageTo
                  if( err ){
                    logLine( transportConfig.config, messageTo, 3, "Error pre-processing message", err );
                    return;
                  }
...
var logLine = exports.transportLayerFunctions.logLine = function( config, messageTo, logLevel, line, errInLogline ){
  ...
  if( errInLogline ){
    ...
    // a hotplate. critical() (which will display on the screen)
      ...
      var l = { error: errInLogline, system: true, logLevel: logLevel, message: "hotCoreTransport.logLine logged: " + line, data: { config: config, messageTo: messageTo } };

      // Attempts to work out the workspaceId
      if( config && config.workspaceId ) l.workspaceId = config.workspaceId;
  ...
  if( config && config.workspaceId ){
    o.workspaceId = config.workspaceId;
  }
