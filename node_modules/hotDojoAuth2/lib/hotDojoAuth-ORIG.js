...
var pageLogin = function(req, res, next){
  ...
  if(! req.session.loggedIn){
    ...
    var extraJses = (new hotCorePage.Jses).add('hotDojoAuth2', 'mainLogin.js');
    var extraCsses = (new hotCorePage.Csses).add('hotDojoAuth2', 'rrl.css');
    var extraVars = new hotCorePage.Vars();

    extraVars.add( 'hotDojoAuth2', 'loggedIn', false );
    extraVars.add( 'hotDojoAuth2', 'login',  typeof( req.session.login )  === 'undefined' ? '' : req.session.login );
    extraVars.add( 'hotDojoAuth2', 'userId', typeof( req.session.userId ) === 'undefined' ? '' : req.session.userId );
...
var pageRegister = function(req, res, next){
    ...
    var extraJses = (new hotCorePage.Jses).add('hotDojoAuth2', 'mainRegister.js');
    var extraCsses = (new hotCorePage.Csses).add('hotDojoAuth2', 'rrl.css');
    var extraVars = new hotCorePage.Vars();
    ...
    // 
    if( req.session.loggedIn){
      extraVars.add('hotDojoAuth2', 'loggedIn', true );
      extraVars.add('hotDojoAuth2', 'login', req.session.login );
      extraVars.add('hotDojoAuth2', 'userId', req.session.userId );
    } else {
      extraVars.add('hotDojoAuth2', 'loggedIn', false );
      extraVars.add('hotDojoAuth2', 'login', typeof( req.session.login ) === 'undefined' ? '' : req.session.login );
      extraVars.add('hotDojoAuth2', 'userId', typeof( req.session.userId) === 'undefined' ? '' : req.session.userId );
