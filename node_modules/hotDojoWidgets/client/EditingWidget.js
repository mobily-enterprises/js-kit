    ...
    postCreate: function(){
      ...
      self.own(
        ...
        topic.subscribe('storeRecordUpdate', function( from, message, remote ){
          ...
          // If it's a remote update (not dealt with with local notify), and if it's not just a repositioning,
          // and if suppressReload is false, then place an overlay on the record
          // NOTE: Changed this, because it cannot only get triggered for "remote" events. It needs
          // to check ANY events
          //if( remote ){
            if( message.storeName === self.storeName && message.objectId == self.recordId && !message.suppressReload  && ! message.justReposition ){
