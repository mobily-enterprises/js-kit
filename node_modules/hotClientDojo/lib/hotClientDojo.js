...
hotplate.hotEvents.on( 'clientPath', 'hotClientDojo', function( done ){
...
hotplate.hotEvents.on( 'pageElementsPerPage', 'hotClientDojo', function( req, pageName, done ){
  ...
  if( pageName === 'hotClientDojo/Welcome') listAsArray.push( 'mainWelcome' );
  if( pageName === 'hotClientDojo/Pick') listAsArray.push( 'mainPick' );
  if( pageName === 'hotClientDojo/container' ) listAsArray.push( 'mainContainer' );

  var requireLine = '';
  requireLine += '<script>\n';
  requireLine += '  // Modules marked as to be loaded by hotplate\n';
  requireLine += '  require(["dojo/topic", "dojo/dom-class", "dojo/_base/window", "messages", ';
  ...
  hotplate.hotEvents.emit( 'dojoModulesPerPage', req, pageName, function( err, dojoModules ){
    ...
    dojoModules.forEach( function( moduleInfo ){
      moduleInfo.result.forEach( function( moduleResult ){
        listAsArray.push( '"' + path.join( hotplate.config.get( 'hotplate.moduleFilesPrefix' ), moduleInfo.module, moduleResult) + '"');
      });
    });

    requireLine += listAsArray.join() + ', "dojo/domReady!"], function(topic, domClass, win ){\n';
...
hotplate.hotEvents.on( 'pageElements', 'hotClientDojo', hotplate.cachable( function( done ){

  var headLines, dojoConfigAssign;

  hotplate.hotEvents.emit( 'enrichDojoConf', hotplate.config.get( 'hotClientDojo.dojoConfig'), function( err, results ){
    if( err ) return done( err );
      ...
      csses: [
        'auth.css', // TODO: Should be "per page"
        'EditableList.css',
        'StoreSelect.css',
        'dgrid/css/skins/' + hotplate.config.get('hotClientDojo.dgrid-theme' ) + '.css',
        'mainAlertBar',
        'AppStoreNotify',
        'AlertBar.css', 'EditingWidget.css','_OverlayMixin.css', 'StoreToggle.css', 'ValidationTextArea.css',
      ] 
    });
  });
}));
...
hotplate.hotEvents.on( 'setRoutes', hotplate.cachable( function( app, done ){
  ...
  // Routes for Dojo's welcome and pick pages
    ...
    hotCorePage.processPageTemplate(
      {
        ...
        csses: (new hotCorePage.Csses() ).add( 'hotClientDojo', 'mainContainer.css' ),
...
var pageWelcome = function(req, res, next){
  ...
  if(! req.session.loggedIn ){
    ...
    hotCorePage.processPageTemplate(
      {
        csses: (new hotCorePage.Csses() ).add('hotClientDojo', 'welcome.css'),
    ...
    var allStores = hotCoreStoreRegistry.getAllStores( function( err, allStores ){
      ...
      allStores.usersWorkspaces.dbLayer.selectByHash( { conditions: { userId: req.session.userId  }  }, {children: true }, function( err, records ){
        ...
        if( records.length !== 1 || ! hotplate.config.get( 'hotCoreMultiHome.escapePick' ) ){
          hotCorePage.processPageTemplate(
            {
              csses: (new hotCorePage.Csses() ).add( 'hotClientDojo', 'pick.css' ),
...
hotplate.hotEvents.on( 'dojoModulesPerPage', 'hotDojoAuth', function( req, pageName, done ){
  ...
  if( pageName === 'hotDojoAuth/Welcome') return done( null, [ 'mainWelcome' ] );
  if( pageName === 'hotDojoAuth/Pick') return done( null, [ 'mainPick' ] );
