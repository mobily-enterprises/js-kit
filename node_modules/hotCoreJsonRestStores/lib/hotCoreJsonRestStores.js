...
exports.get = function( done ){
  ...
  hotplate.hotEvents.emitCollect('sharedFunctions', function( err, sharedFunctions) {
    if( err ){
      ...
      classes.HotSchemaMixin = declare( Object, {
      ...
      classes.HotStoreMixin = declare( Object, {
        ...
        broadcastStoreChanges: function( request, type, objectId, object, options, cb ){
          ...
          if( typeof( options.userId ) === 'undefined' ){
            //userId = ( request.session && request.session.userId ) || object.userId || undefined;
            userId = object.userId || undefined;
          ...
          //return cb( null );
          hotplate.hotEvents.emitCollect('cometBroadcast', userId, tabId, makeTabIdHash, message, function( err ){
            if( err ) return cb( err );

            // No alsoNotify: nothing else to do
            if( ! self.alsoNotify ) return cb( null );

            async.each(
              self.alsoNotify,
              function( alsoStore, cb ){
                message.storeName = alsoStore;
                message.alsoNotify = true;
                message.keepForm = true;
        ...
        afterEverything: function f( request, method, done ){
          ...
          // in hotCoreJsonRestStores
          this.inheritedAsync( f, arguments, function( err ){
            if( err ) return done( err );

            if( method === 'getQuery' || method === 'get' ) return done( null );

            var d = request.data.preparedDoc;

            var m;
            switch( method ){
              case 'put':
                if( request.putNew ) m = 'storeRecordCreate';
                else m = 'storeRecordUpdate';
              break;
              case 'post':
                m = 'storeRecordCreate';
              break;
