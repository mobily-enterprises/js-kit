  ...
  hotplate.hotEvents.emit('sharedFunctions', function( err, sharedFunctions) {
    if( err ){
      ...
      classes.HotStoreMixin = declare( null, {
        ...
        // `request` is passed to work out `tabId` and `userId` if they are not passed
        // `justReposition` and `beforeId` are not worked out from headers if they are not passed as they
        ...
        broadcastStoreChanges: function( request, type, objectId, object, options, cb ){
          ...
          message = { type: type, storeName: this.storeName, objectId: objectId, object: object, justReposition: justReposition, before: before };

          hotplate.hotEvents.emit('cometBroadcast', userId, tabId, makeTabIdHash, message, cb );
        },

        // request._req.session.userId -- session's user ID
        // request._req -- request's tab ID (from headers)
