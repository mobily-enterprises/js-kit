  ...
  hotplate.hotEvents.emit('sharedFunctions', function( err, sharedFunctions) {
    if( err ){
      ...
      classes.HotStoreMixin = declare( null, {
        ...
        // * `options.userId`
        // * `options.tabId`
        // * `options.putBefore` and
        // * `options.putDefaultPosition`
        ...
        broadcastStoreChanges: function( request, type, objectId, object, options, cb ){
          ...
          // userId, tabId, putBefore and putDefaultPosition are inferred from `request` if 
          // they weren't passed
          if( typeof( options.userId ) === 'undefined' ){
            options.userId = request._req && request._req.session && request._req.session.userId || undefined;
          }
          if( typeof( options.tabId ) === 'undefined' ){
            options.tabId = this._getTabId( request._req );
          }
          if( typeof( options.putBefore ) === 'undefined' ){
            options.putBefore = request.options.putBefore;
          } 
          if( typeof( options.putDefaultPosition ) === 'undefined' ){
            options.putDefaultPosition = request.options.putDefaultPosition;
          } 
          ...
          for( var k in options ) message[ k ] = options[ k ];

          hotplate.hotEvents.emit('cometBroadcast', options.userId, options.tabId, makeTabIdHash, message, cb );
        },
