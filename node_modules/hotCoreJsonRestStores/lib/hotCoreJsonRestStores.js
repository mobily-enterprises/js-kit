  ...
  hotplate.hotEvents.emit('sharedFunctions', function( err, sharedFunctions) {
    if( err ){
      ...
      classes.HotStoreMixin = declare( null, {
        ...
        // `options` can have: `options.userId`, `options.tabId`, `options.beforeId`
        // `request` is used to work out `tabId` and `userId` if they are not set in options
        // `beforeId` is not worked out from headers if it's not passed
        ...
        broadcastStoreChanges: function( request, type, objectId, object, options, cb ){
          ...
          var userId, tabId, putBefore, putDefaultPosition;
          var makeTabIdHash;
          var message;
          ...
          // putBefore and putDefaultPosition are also inferred from `request` if
          // they weren't passed
          putBefore = options.putBefore;
          if( typeof( putBefore ) === 'undefined' ){
            putBefore = request.options.putBefore;
          } 
          putDefaultPosition = options.putDefaultPosition;
          if( typeof( putDefaultPosition ) === 'undefined' ){
            putDefaultPosition = request.options.putDefaultPosition;
          } 
          ...
          message = { type: type, storeName: this.storeName, objectId: objectId, object: object };

          // Enrich message with putBefore and putDefaultPosition if present
          if( typeof( putBefore ) !== 'undefined' ){
            message.putBefore = putBefore;
          }
          if( typeof( putDefaultPosition ) !== 'undefined' ){
            message.putDefaultPosition = putDefaultPosition;
          }
          
          hotplate.hotEvents.emit('cometBroadcast', userId, tabId, makeTabIdHash, message, cb );
        },

        // `request` here is used for one option:
...
/*
          // beforeId  needs to be passed to the broadcasting function, which
          // does not work it out automatically.
