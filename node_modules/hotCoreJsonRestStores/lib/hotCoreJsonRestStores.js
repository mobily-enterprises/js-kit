  ...
  hotplate.hotEvents.emit('sharedFunctions', function( err, sharedFunctions) {
    if( err ){
      ...
      classes.HotStoreMixin = declare( null, {
        ...
        // `options` can have: `options.userId`, `options.tabId`, `options.beforeId`, `options.justReposition`
        // `request` is used to work out `tabId` and `userId` if they are not set in options
        // `justReposition` and `beforeId` are not worked out from headers if they are not passed as
        // `suppressReload` is an extra option
        ...
        broadcastStoreChanges: function( request, type, objectId, object, options, cb ){
          ...
          var userId, tabId, beforeId, justReposition;
          var suppressReload;
          var before;
          var makeTabIdHash;
          var message;
          ...
          // userId and tabId are inferred from `request` if 
          ...
          // If beforeId is passed, then the `before` entry in the message is created
          // as an empty object with just the property set
          // (Unless it's `null`, and then `before` needs to be null too, which means "last spot")
          if( typeof( options.beforeId ) !== 'undefined' ){
            if( options.beforeId === null ) before = null;
            else { before = {}; before[ this.idProperty ] = options.beforeId; }
          }

          // justReposition is a flag, easy one
          justReposition = !! options.justReposition;

          // suppressReload
          suppressReload = !! options.suppressReload;
          ...
          message = { type: type, storeName: this.storeName, objectId: objectId, object: object, justReposition: justReposition, before: before, suppressReload: suppressReload };

          hotplate.hotEvents.emit('cometBroadcast', userId, tabId, makeTabIdHash, message, cb );
        },

        // `request` here is used for two options:
        // request.options.beforeId
        // request.options.justReposition
        _broadcast: function( request, type, objectId, object, cb ){

          var options = {};

          // beforeId and justReposition need to be passed to the broadcasting function, which
          // does not work them out automatically.
          // `tabId` and `userId` will be worked out from `request`
          if( typeof( request.options.beforeId ) !== 'undefined' ) options.beforeId = request.options.beforeId;
          if( request.options.justReposition) options.justReposition = request.options.justReposition;
