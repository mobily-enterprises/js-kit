...
, stores
  ...
  //r = declare( [ BorderContainer ], {
    ...
    postCreate: function(){
      ...
      self.listWidget = new L( { collection: self.store, sort: self.initialSort, queryOptions: self.initialQueryOptions, query: self.initialQuery, region: 'center' } );
      ...
      self.own(
        ...
        // If a record is edited, and filter is not empty, refresh
        topic.subscribe('storeRecordUpdate', function( from, message, remote ){
          if( message.storeName == self.storeName ){
            var refresh = false;
            for( var k in self.listWidget.get('query') ) refresh = true;
            if( refresh ) self.listWidget.refresh( { keepScrollPosition: true } );
          }
        }),
