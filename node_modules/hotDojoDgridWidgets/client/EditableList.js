...
, _OverlayMixin
  ...
  r = declare( [ BorderContainer, _OverlayMixin ], {
    ...
    completeWidget: function( widget ){

      var self = this;

      widget.on('successfulsubmit', function( e ){

        // if self.refreshOnSubmit is set, then a successful submit
        // will refresh the list
        if( self.refreshOnSubmit ) self.listWidget.refresh();

        // Re-emit the event on a local basis so that this widget can catch it
        self.emit( 'successfulsubmit', e );
      });
    ...
    addingConstructorInDialog: function( dialogTitle ){
      ...
      self.completeWidget( addingWidget );
    ...
    postCreate: function(){
      ...
      // Overlay status as RELOAD by default -- it will be taken out by a dgrid-refresh-complete
      ...
      // and a region were passed (it means, "create and place it")
      if( self.AddingConstructor && self.addingWidgetRegion ){
        ...
        addingWidget.region = self.addingWidgetRegion;
    ...
    _editInDialog: function( row ){
      ...
      self.completeWidget( editingWidget );
