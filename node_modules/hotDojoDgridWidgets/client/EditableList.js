...
, stores
  ...
  r = declare( [ BorderContainer ], {
    ...
    /*
    // "Nice and simple" proved not functional
    templateString: ''+
                    '<div>\n'+
    ...
    */
    ...
    postCreate: function(){
      ...
      /*
      // NOTE: this is commented out because successfulsubmit is
      // a LOCAL event, from a form that just entered some data.
      // It's OK to close an editing dialog box with this event.
      // However, if it's used to power the `refreshOnSubmit` event,
      // it will only refresh for the LOCAL browser, which means
      // that the local tab will reload on save, but other tabs/users
      // will not, which is inconsistent
      ...
      */
      ...
      self.listWidget = new L( { store: self.store, sort: self.initialSort, queryOptions: self.initialQueryOptions, region: 'center' } );
      //
      ...
      self.own(
        ...
        aspect.around( self.listWidget, 'refresh', function( refresh ){
          return function( options ){
            ...
            console.log("REFRESH REQUESTED", self.listWidget.store.storeName);
            if( ! self.listWidget._inRefresh ){
              console.log("REFRESH EXECUTED", self.listWidget.store.storeName);
