...
, stores
  ...
  //r = declare( [ BorderContainer ], {
    ...
    // Refresh parameters
    refreshOnChange: false, /* Set to true for sorted dgrid where editing will change element's position */
    ...
    postCreate: function(){
      ...
      self.listWidget = new L( { collection: collection, region: 'center' } );
      ...
      self.own(
        ...
        // TODO dstore: define this if
        // If a new record is created, deleted/updated, refresh
        topic.subscribe('storeRecordCreate,storeRecordUpdate,storeRecordDelete', function( from, message, remote ){
           //console.log("AH!", message.storeName );
           //if( message.storeName == 'workspacesContactsCategories' && self.store.storeName == 'workspacesContactsCategories' ) SSSS = self.store;
          //if( message.storeName == self.storeName && ! message.suppressReload && self.refreshOnChange ){
          //if( message.storeName == self.storeName ) { //&& ! message.suppressReload && self.refreshOnChange ){
          if( message.storeName == self.storeName && ! message.suppressReload ) {

            //self.listWidget._setCollection( self.listWidget.collection);
            self.listWidget._renderedCollection._collectionCache = {}
            self.listWidget.refresh( { keepScrollPosition: true } );
          }
        }),
      ...
      if( self.ExtraWidgetConstructor ){
        ...
        if( self.extraWidgetRegion ){
          ...
          //self.addChild( self.extraWidget );
        }
      }
      ...
      if( self.ButtonsConstructor ){
        ...
        self.own( on( self.buttonsNode, mouse.enter, function(e){
          self._onButtonsNode = true;
        }) );

        self.own( on( self.buttonsNode, mouse.leave, function(e){
          self._onButtonsNode = false;
          hideButtonsNode();
        }) );

        self.own( on( self.listWidget.domNode, mouse.enter, function(e){
          self._onWidget = true;
        }) );

        self.own( on( self.listWidget.domNode, mouse.leave, function(e){
