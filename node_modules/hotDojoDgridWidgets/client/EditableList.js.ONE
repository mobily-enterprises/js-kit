      ...
      postCreate: function(){
        ...
        if( self.editButton ){
          self.editButton.on('click', function(e){
            var buttonsWidget = e.srcElement.offsetParent;
            var row = self.row( buttonsWidget.getAttribute( 'rowId' )  );         
            domStyle.set( buttonsWidget, { display: 'none' } );
            self._editRow( row );
          });
        }
        ...
        if( self.deleteButton ){
          self.deleteButton.on('click', function( deleteEvent ){
            ...
            var row = self.row( deleteEvent.srcElement.offsetParent.getAttribute( 'rowId' ) );         
        ...
        self.buttonWidgets = put( 'div.button-widgets' );
        if( self.editButton ) put( self.buttonWidgets, self.editButton.domNode );
        if( self.deleteButton ) put( self.buttonWidgets, self.deleteButton.domNode );
        if( self.editButton || self.deleteButton ){
          put( document.body, self.buttonWidgets );
          domStyle.set( self.buttonWidgets, { display: 'none', opacity: '0' } );
        }
        ...
        self.listWidget.on( mouse.enterRow, function(e){
          ...
          if( self.editButton || self.deleteButton ){
            domStyle.set( self.buttonWidgets, { 'transition-duration' : 0 } );
            domStyle.set( self.buttonWidgets, { display: 'block', opacity: '0' } );
            var geo = domGeometry.position( row.element, false );
            domStyle.set( self.buttonWidgets, 'top', (geo.y + geo.h - 4 ) + 'px' );
            domStyle.set( self.buttonWidgets, 'left', geo.x + 'px' );
            domStyle.set( self.buttonWidgets, { 'transition-duration': '0.3s' } );
            domStyle.set( self.buttonWidgets, { opacity: '1' } );

            // Since this is not part of the DOM, the buttons
            // need to be told what row they are referring to, as
            // self.row will fail...
            self.buttonWidgets.setAttribute( 'rowId', row.id );
        ...
        // self.own( on( self.domNode, dojoMouse.leave, function(e){
          // console.log("TEST RIGHT NOW: ");
          // console.log( self._onButtonWidgets );
          // if( ! self._onButtonWidgets && ( self.editButton || self.deleteButton ) ){
            //domStyle.set( self.buttonWidgets, { 'transition-duration' : 0 } );
            // domStyle.set( self.buttonWidgets, { display: 'none' } );
        //    domStyle.set( self.buttonWidgets, { display: 'none' } );
          //}
        // } );
        ...
        self.own( on( self.domNode, dojoMouse.leave, function(e){
          console.log("LEFT widget, quit! ");
          self._onWidget = false;
          domStyle.set( self.buttonWidgets, { display: 'none' } );
        }) );

        self.own( on( self.buttonWidgets, dojoMouse.enter, function(e){
          console.log("ON button, entered! ");
          self._onButtonWidgets = true;
          domStyle.set( self.buttonWidgets, { display: 'block' } );
        }) );
        ...
        */
        ...
        self.own( on( self.buttonWidgets, dojoMouse.leave, function(e){
          console.log("LEFT button, quit! ");
          self._onButtonWidgets = false;
          if( ! self._onWidget ){
            domStyle.set( self.buttonWidgets, { display: 'none' } );
          }
        }) );
