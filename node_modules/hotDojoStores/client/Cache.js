...
define(["dojo/_base/lang","dojo/_base/Deferred" /*=====, "../_base/declare", "./api/Store" =====*/],
...
var Cache = function(masterStore, cachingStore, options){
	...
	return lang.delegate(masterStore, {
      ...
      if( this.cacheQuery ) return cachingStore.query( query, directives ); // MERC
		...
		add: function(object, directives){
			return Deferred.when(masterStore.add(object, directives), function(result){
				...
				// cachingStore.add(typeof result == "object" ? result : object, directives); // MERC
				cachingStore.add(typeof result == "object" && result !== null ? result : object, directives);
				return result; // the result from the add should be dictated by the masterStore and be unaffected by the cachingStore
			});
		},
		put: function(object, directives){
      if( ! this.cacheQuery ) // MERC. So that if the Json rest put fails, the cache is still intact and queries will work
			// first remove from the cache, so it is empty until we get a response from the master store
			cachingStore.remove((directives && directives.id) || this.getIdentity(object));
			return Deferred.when(masterStore.put(object, directives), function(result){
				// now put result in cache
				// cachingStore.put(typeof result == "object" ? result : object, directives); // MERC
