define(["dojo/_base/kernel", "dojo/_base/lang", "dojo/_base/Deferred", "dojo/_base/array", "dojo/when" /*=====, "./api/Store" =====*/
], function(kernel, lang, Deferred, array, when /*=====, Store =====*/){
...
var Observable = function(/*Store*/ store){
	...
	store.query = function(query, options){
		...
		if(results && results.forEach){
			...
			var newQueryTo = options.start + options.count; // Added by MERC, needed later
					...
					queryUpdaters.push(queryUpdater = function(changed, existingId){
						Deferred.when(results, function(resultsArray){
							...
							if( changed !== undef ){
								...
								if( ownPosition == -1 ){
									...
									if( options.before ){
										...
										var beforePosition = findObjectInResult( store.getIdentity( options.before ) )
									...
									// CASE 3b: it's an add, and query.placeNew is "last" 
									...
									// made 9and therefore observed) yet
									} else if( options.placeNew === 'last' || options.before === null ){
										...
										when( results.total ).then( function( grandTotal ) {
											...
											// This query has the last result: append to it
											// console.log("PARAMS: " + options.start + "-" + newQueryTo + ' ---  grandTotal ' + grandTotal );
											if( newQueryTo >= grandTotal ){
							...
							function tellAllListeners( element, removedFrom, insertedInto ){
								//console.log("Telling all listeners: ");
								//console.log( element );
