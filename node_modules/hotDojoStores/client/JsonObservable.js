define(["dojo/_base/kernel", "dojo/_base/lang", "dojo/when", "dojo/_base/array", "dojo/when" /*=====, "./api/Store" =====*/], function(kernel, lang, when, array, when /*=====, Store =====*/){
...
var Observable = function(/*Store*/ store){
	...
	store.query = function(query, options){
		...
		if(results && results.forEach){
			...
			results.newQueryTotal = options.start + options.count;    // Added by MERC
			results.options = lang.mixin( {}, options );              // Added by MERC

			results.observe = function(listener, includeObjectUpdates){

				var newQueryTotal = results.newQueryTotal; // ADDED BY MERC
				var options = results.options;             // ADDED BY MERC
						...
						when(results, function(resultsArray){
							...
							if( changed !== undef ){
								...
								if( ownPosition == -1 ){
									...
									// made and therefore observed yet)
									} else if( options.placeNew === 'last' ||
										       options.placeNew === undefined ||
                                                 ( typeof( methodOptions ) === 'object' && 
                                                   methodOptions !== null && 
                                                   methodOptions.before === null
                                                  ) 
                                            ){
										...
										when( results.total ).then( function( grandTotal ) {
											...
											// If there are no limits to the query, OR if the new element will 
											// be outside the range of 
											// the current query, OR if methodOptions.before is null, add it
                                            // FIXME: it should check if it's within the last page of 
                                            // the shown fields, so that if you are for example
